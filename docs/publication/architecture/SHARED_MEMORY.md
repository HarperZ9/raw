...
SkyrimBridge writes all game state to a named memory-mapped file every frame. External applications can read this data without touching the game process.
...
| Version | `1` |
| Shared Memory Name | `SkyrimBridge_GameState` |
| Event Name | `SkyrimBridge_DataReady` |
...
var mmf = MemoryMappedFile.OpenExisting("SkyrimBridge_GameState");
...
try:
    shm = mmap.mmap(-1, 4096, "SkyrimBridge_GameState", access=mmap.ACCESS_READ)
    ...
    pass  # SkyrimBridge not running
```

## Event Signaling

After each frame write, SkyrimBridge signals the named event `SkyrimBridge_DataReady`. External apps can wait on this event for frame-synchronous reads:

```c
HANDLE hEvent = OpenEventW(SYNCHRONIZE, FALSE, L"SkyrimBridge_DataReady");
...
The standalone `SkyrimBridge_ENB.dllplugin` uses this same shared memory interface. It reads `SkyrimBridge_GameState` and serves parameters to ENB shaders via `ENBGetParameter` (pull model), providing a secondary data path that doesn't require SKSE.
