...
// Live-bridge control dispatch: out-of-game control.ini -> effect toggles.
static void ApplyLiveControl(const std::string& k, const std::string& v)
{
...
static void DoFrameUpdate()
{
    ...
    // ── Live bridge (coherence membrane, Tier 0.1): emit per-frame telemetry,
    //    apply out-of-game control.ini. Lazy-init; throttled file IO. ──
    {
        static bool s_liveInit = false;
        if (!s_liveInit) {
            SB::LiveBridge::Get().Initialize("Data/SKSE/Plugins/RAW/live");
            SB::LiveBridge::Get().SetEnabled(true);
            s_liveInit = true;
        }
        SB::LiveFrameStats ls;
        const auto& hr = SB::LuminanceHistogram::Get().GetResult();
        ls.lumaMean = hr.avgLuminance; ls.lumaMin = hr.minLuminance; ls.lumaMax = hr.maxLuminance;
        ls.gpuMs = SB::GPUProfiler::Get().GetTotalGpuMs();
        ls.fps   = (ls.gpuMs > 0.f) ? 1000.f / ls.gpuMs : 0.f;
        ls.depthValid = D3D11Hook::GetGameDepthSRV() != nullptr;
        uint32_t ef = 0;
        if (SB::GTAORenderer::Get().IsEnabled())          ef |= 1u;
        if (SB::SSGIRenderer::Get().IsEnabled())          ef |= 2u;
        if (SB::SSRRenderer::Get().IsEnabled())           ef |= 4u;
        if (SB::ContactShadowRenderer::Get().IsEnabled()) ef |= 16u;
        if (SB::SkylightingRenderer::Get().IsEnabled())   ef |= 32u;
        ls.enableFlags = ef;
        ls.sawNaN = SB::TextureDump::LastDumpSawNaN();  // Tier 1.1 (sampled on F5 dump)
        SB::LiveControl cmd = SB::LiveBridge::Get().Tick(ls);
        if (cmd.seq >= 0) {
            for (auto& kv : cmd.pairs) ApplyLiveControl(kv.first, kv.second);
            if (cmd.reload) SB::ReloadAllShaders();
