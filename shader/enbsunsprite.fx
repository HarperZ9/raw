                ...
                uniform float ghostOffset, uniform float ghostScale)
{
    ...
    float3 sunCol;
    float sunInt = GetSunIntensity(ui_GlobalIntensity * ui_GhostIntensity, sunCol);
    OUT.sunCol = sunCol;
    OUT.sunInt = sunInt;
    OUT.centered = pos.xy;
               ...
               uniform float anamOffset)
{
    ...
    float3 sunCol;
    float sunInt = GetSunIntensity(ui_GlobalIntensity * ui_AnamorphicIntensity, sunCol);
    OUT.sunCol = sunCol;
    OUT.sunInt = sunInt;
    OUT.centered = pos.xy;
...
VS_OUT VS_Starburst(float4 pos : SV_POSITION, float2 txcoord : TEXCOORD0)
{
    ...
    float3 sunCol;
    float sunInt = GetSunIntensity(ui_GlobalIntensity * ui_StarburstIntensity, sunCol);
    OUT.sunCol = sunCol;
    OUT.sunInt = sunInt;
    OUT.centered = pos.xy;
...
VS_OUT VS_SunGlare(float4 pos : SV_POSITION, float2 txcoord : TEXCOORD0)
{
    ...
    float3 sunCol;
    float sunInt = GetSunIntensity(ui_GlobalIntensity * ui_SunGlareIntensity, sunCol);
    OUT.sunCol = sunCol;
    OUT.sunInt = sunInt;
    OUT.centered = pos.xy;
...
VS_OUT VS_Hoop(float4 pos : SV_POSITION, float2 txcoord : TEXCOORD0)
{
    ...
    float3 sunCol;
    float sunInt = GetSunIntensity(ui_GlobalIntensity * ui_HoopIntensity, sunCol);
    OUT.sunCol = sunCol;
    OUT.sunInt = sunInt;
...
// Ghost PS — N-gon shaped ghost with CA
float4 PS_Ghost(VS_OUT IN, uniform float ghostWeight) : SV_Target
{
    ...
    // Simple radial shape (circle with N-gon optional)
    ...
    float3 caShape;
    caShape.r = 1.0 - saturate(length(c - caVec * 0.5));
    caShape.g = inside;
    caShape.b = 1.0 - saturate(length(c + caVec * 0.5));

    // Feathering + weight
    float3 feather = smoothstep(0.0, 0.05, caShape);
    float3 weight = smoothstep(6.0 * pow(ghostWeight, 3.0), 0.0, 1.0 - caShape);

    float3 color = IN.sunInt * IN.sunCol * feather * weight;
...
float4 PS_Anam(VS_OUT IN) : SV_Target
{
    ...
    float2 grad = 1.0 - abs(IN.centered);
    float hFade = max(grad.x, 0.0);
    float vShape = pow(max(grad.y, 0.0), 3.0);

    float3 color = hFade * vShape * IN.sunInt * IN.sunCol;
    return float4(max(color, 0.0), 1.0);
}
...
float4 PS_Starburst(VS_OUT IN) : SV_Target
{
    ...
    // Radial falloff
    float falloff = pow(saturate(1.0 - dist), ui_StarburstFalloff);

    float3 color = starMask * falloff * IN.sunInt * IN.sunCol;
    return float4(max(color, 0.0), 1.0);
}
...
float4 PS_SunGlare(VS_OUT IN) : SV_Target
{
    ...
    float3 color = glow * IN.sunInt * IN.sunCol;
    return float4(max(color, 0.0), 1.0);
}
