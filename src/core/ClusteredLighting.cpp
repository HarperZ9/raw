...
{
    auto CompileCS = [&](const char* name, const char* source, const char* entry,
                         ID3D11ComputeShader** outCS) -> bool
    {
        ID3DBlob* blob = ShaderLoader::Compile(name, source, entry, "cs_5_0", D3DCOMPILE_OPTIMIZATION_LEVEL3);
        if (!blob) {
            SKSE::log::error("ClusteredLighting CS compile failed: {}", name);
            return false;
        }
    ...
    if (!CompileCS("Cluster_Build", kClusterBuildCS, "main", &m_clusterBuildCS))
        return false;

    if (!CompileCS("Cluster_Cull", kClusterCullCS, "main", &m_clusterCullCS))
        return false;
