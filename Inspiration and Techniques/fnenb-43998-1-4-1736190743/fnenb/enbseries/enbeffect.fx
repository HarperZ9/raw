...
float Param01 <string UIName="DAY";  string UIWidget="spinner";  float UIMin=0.0;  float UIMax=0.0;> = {0.0};
...
float Param02 <string UIName="NIGHT";  string UIWidget="spinner";  float UIMin=0.0;  float UIMax=0.0;> = {0.0};
...
float Param03 <string UIName="INTERIORS";  string UIWidget="spinner";  float UIMin=0.0;  float UIMax=0.0;> = {0.0};
...
float Param04 <string UIName="EYE ADAPTATION";  string UIWidget="spinner";  float UIMin=0.0;  float UIMax=0.0;> = {0.0};
...
float Param06 <string UIName="NIGHT VISION GLOBAL ON/OFF";  string UIWidget="spinner";  float UIMin=0.0;  float UIMax=0.0;> = {0.0};
//{ Parameters
	//{ Enable
		bool EENE_Enable <
		...
		> = {true};
	//}
float Param07 <string UIName="NIGHT VISION CALIBRATION";  string UIWidget="spinner";  float UIMin=0.0;  float UIMax=0.0;> = {0.0};
	//{ Calibration Display
...
Texture2D TextureColor;//color
Texture2D TextureBloom;//bloom skyrim
Texture2D TextureAdaptation;//adaptation skyrim
Texture2D TexturePalette;//palette enb
Texture2D TextureLens;//lens
...
static float nighteye_t = 0.0;
static float nighteye_time = -1.0;

float4 PS_D6EC7DD1(VS_OUTPUT_POST IN, float4 vPos : SV_Position) : SV_Target
{
	...
	float EENE_warpT = Params01[5].w;
	float4 EENE_Reg1 = Params01[0];
	float4 EENE_Reg2 = Params01[1];
	float4 EENE_Reg3 = Params01[2];
	float4 EENE_Reg4 = Params01[3];
	float4 EENE_Reg5 = Params01[4];
	...
	if (EENE_Enable)
	{
		...
		EENE_nightEyeT = lerp(lerp(nightFactor, dayFactor, ENightDayFactor), interiorFactor, EInteriorFactor).x;
		...
		// Warping
			...
			EENE_warpT *= clamp(nighteye_t, 0.0, 1.0);
   ...
   //r1=tex2D(Sampler0, _v0.xy); //color
   r1=TextureColor.Sample(Sampler0,_v0.xy);
	...
	//apply original
    r0.x=1.0/_c2.y;
    r1=TextureAdaptation.Sample(Sampler1,_v0);/*tex2D(_s2, _v0);*/
    r0.yz=r1.xy * _c1.y;
    r0.w=1.0/r0.y;
    r0.z=r0.w * r0.z;
    r1=TextureColor.Sample(Sampler0, _v0);/*tex2D(Sampler0, _v0);*/
    r1.xyz=r1 * _c1.y;
...
//    r2=tex2D(_s3, _v0);//enb bloom
    r2=TextureBloom.Sample(Sampler1, _v0);/*tex2D(Sampler1, _v0);*/ //skyrim bloom
    r2.xyz=r2 * _c1.y;
    r2.xyz=r0.x * r2;
    r1.xyz=r1 * r0.z + r2;
	...
	//adaptation in time
	float4	Adaptation=TextureAdaptation.Sample(Sampler1, 0.5 ); /*tex2D(_s4, 0.5);*/
   ...
   float EyeAdapt      = grayValue( TextureAdaptation.Sample(Sampler1, 0.5).xyz /*tex2D( _s4, 0.5 ).xyz*/ );
...
//-/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
 ...
 if (ENABLE_KABLOOM==true)
  {
    	color.rgb = kaBloom(Sampler1,TextureBloom,_v0,color.rgb,InBlack,InWhite,InGamma,OutBlack,OutWhite,BloomBrightness,BloomContrast,BloomSaturation,BloomSensitivity,BloomIntensity,RGBBalance,RGBWeights,BlendMode,BlendAmount,BlendOpacity);
  }

//-/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	
    float4	xcolorbloom=TextureBloom.Sample(Sampler1, _v0.xy);/*tex2D(_s3, _v0.xy);*/ //bloom

	color.xyz+=xcolorbloom.xyz*ENBParams01.x;

	float4 xcolorlens=TextureLens.Sample(Sampler1, _v0.xy);
	color.xyz+=xcolorlens.xyz*ENBParams01.y;
...
color.rgb  = lerp( TexturePalette.SampleLevel(Sampler1, CLut_UV.xy, CLut_UV.z).rgb, TexturePalette.Sample(Sampler1, CLut_UV.xy + float2(CLut_pSize.y, 0), CLut_UV.z).rgb, color.b - CLut_UV.w);
#endif //E_CC_PALETTE
	...
	if (EENE_Enable)
	{
		...
		if (Params01[5].w > 0.0)
		{
			...
			float inverseScreenSize = 1.0 / ScreenSize.x;
			...
			//{ Sharpening
				...
				if (EENE_sharpeningEnable)
				{
					...
					for (int i = 0; i < EENE_sharpeningSamples; i++)
					{
						...
						sharpColor += TextureColor.SampleLevel(Sampler0, sampleCoord.xy, sampleCoord.w).rgb; /*tex2Dlod(Sampler0, sampleCoord);*/
					}
			...
			//{ Blur
				...
				if (EENE_blurEnable)
				{
					...
					for (int i = 0; i < EENE_blurSamples; i++)
					{
						...
						blurColor += TextureColor.SampleLevel(Sampler0, sampleCoord.xy, sampleCoord.w).rgb; /*tex2Dlod(Sampler0, sampleCoord).rgb; */
					}
			...
			//{ Bloom
				...
				if (EENE_enableBloom)
				{
					...
					float3 nightEyeBloom = TextureBloom.Sample(Sampler1, _v0.xy).rgb;/*tex2D(_s3, _v0);*/
			...
			if(Params01[5].w > 0.098 && Params01[5].w < 0.09805) {
				nighteye_t += 0.1;
			} else {
				nighteye_t = 0.0;
			}

			color.xyz = lerp(color.xyz, nightEye.xyz, clamp(nighteye_t, 0.0, 1.0));
		}
...
technique11 Shader_D6EC7DD1 <string UIName="Suki";>
//#else
...
//#endif
{
	pass p0
	{
		...
		SetPixelShader ( CompileShader (ps_5_0, PS_D6EC7DD1() ) );
	}
}
