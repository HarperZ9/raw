...
int _spcSBW < string UIName = ""; string UIWidget = "spacer"; int UIMin = 0; int UIMax = 0; > = 0;
int _hdrSBW < string UIName = "======= WEATHER BLOOM (SkyrimBridge) ======="; string UIWidget = "spacer"; int UIMin = 0; int UIMax = 0; > = 0;
...
float3 ApplyPointLightBloom(float3 bloom, float2 UV)
{
    ...
    // Accumulate bloom contribution from all tracked point lights
    float bloomRadius = UIPLB_RadiusScale * 0.15;  // Screen-space bloom radius

    [unroll]
    for (int i = 0; i < 3; i++)  // SkyrimBridge tracks up to 3 nearby lights
    {
        SB_PointLightBloom plb = SB_GetPointLightBloom(i);

        if (plb.intensity < 0.001 || !plb.onScreen) continue;

        // Distance from this pixel to light's screen position
        float dist = length(UV - plb.screenUV);
        float falloff = saturate(1.0 - dist / (plb.screenRadius * UIPLB_RadiusScale));
        falloff = falloff * falloff;  // Quadratic falloff

        // Color bleed: how much of the light's color affects bloom
        float3 lightBloom = plb.color * plb.intensity * falloff;
