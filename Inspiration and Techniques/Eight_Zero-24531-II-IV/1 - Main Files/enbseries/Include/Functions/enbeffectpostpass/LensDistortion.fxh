...
#define fFisheyeDistortion lerp(LENS_CA * 0.00166, (LENS_CA * 0.00166) + LENS_DIST * (LENS_DIST > 0.0 ? 0.003 : 0.0015), LENS_DIST == 0 ? 0.0 : 1.0)
#define fFisheyeZoom lerp(0.5, 0.5 + LENS_DIST * (LENS_DIST > 0.0 ? 0.00076 : 0.00019), ENABLE_DISTORTION)

// Define screen texture with mirror tiles
SAMPLER(SamplerColor, LINEAR, Mirror)
...
float4 PS_Dist(float4 vpos : SV_Position, float2 texcoord : TEXCOORD) : SV_Target0
{
    ...
    {
        color = TextureColor.Sample(SamplerLinear, texcoord);
        return color;
    }
    ...
    color = TextureColor.Sample(SamplerLinear,Coords);

    return color;
}

// Fisheye Code by Marty McFly
float4 PS_TVDist(float4 vpos : SV_Position, float2 texcoord : TEXCOORD) : SV_Target0
{
    float4 coord=0.0;
    coord.xy=texcoord.xy;
    coord.w=0.0;
    ...
    float4 color;
    if(!ENABLE_CRT || !TV_OVERLAY || !TV_OVERLAY_DIST)
    {
        color = TextureColor.Sample(SamplerLinear, texcoord);
        return color;
    }
...
{
    if(LENS_CA == 0 || !ENABLE_DISTORTION || !ENABLE_LENS) BluredImage = TextureColor.Sample(SamplerLinear, texcoord).rgb;
    else
    {
        // Grab Aspect Ratio
        ...
        {
            BluredImage = TextureColor.Sample(SamplerColor, texcoord).rgb;
        }
        else
        {
            for (int P = 0; P < Samples && P <= 48; P++)
            {
                ...
                BluredImage += Spectrum(CurrentSample) * TextureColor.SampleLevel(SamplerColor,  Position, 0).rgb;
            }
