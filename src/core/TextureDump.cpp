...
// ── Coherence membrane (Tier 1.1): float-precision NaN/Inf witness ──────────
static bool g_lastDumpSawNaN = false;

static void WriteTextureInspect(const std::string& filepath, DXGI_FORMAT fmt,
                                uint32_t w, uint32_t h, bool hasNan, bool hasInf,
                                uint32_t nanCount, int fx, int fy)
{
    try {
        auto dir = s_outputDir.parent_path() / "live";
        std::error_code ec; std::filesystem::create_directories(dir, ec);
        std::ofstream f(dir / "texture_inspect.jsonl", std::ios::app);
        if (!f.is_open()) return;
        std::string name = std::filesystem::path(filepath).stem().string();
        f << "{\"file\":\"" << name << "\""
          << ",\"format\":" << static_cast<int>(fmt)
          << ",\"w\":" << w << ",\"h\":" << h
          << ",\"has_nan\":" << (hasNan ? "true" : "false")
          << ",\"has_inf\":" << (hasInf ? "true" : "false")
          << ",\"nan_count\":" << nanCount
          << ",\"first_nan\":[" << fx << "," << fy << "]"
          << ",\"numeric_ok\":" << ((hasNan || hasInf) ? "false" : "true") << "}" << static_cast<char>(10);
    } catch (...) { /* never disturb the dump */ }
}

bool TextureDump::LastDumpSawNaN() { return g_lastDumpSawNaN; }
                                 ...
                                 ID3D11Texture2D* tex, const std::string& filepath)
{
    ...
    // ── Tier 1.1: bitwise NaN/Inf scan BEFORE the 8-bit conversion discards it ──
    {
        const bool isFloatFmt =
            desc.Format == DXGI_FORMAT_R32_FLOAT || desc.Format == DXGI_FORMAT_R16_FLOAT ||
            desc.Format == DXGI_FORMAT_R16G16B16A16_FLOAT || desc.Format == DXGI_FORMAT_R11G11B10_FLOAT;
        if (isFloatFmt) {
            bool hasNan = false, hasInf = false; uint32_t nanCount = 0; int fx = -1, fy = -1;
            auto mark = [&](uint32_t mx, uint32_t my){ if (!hasNan) { fx = (int)mx; fy = (int)my; } hasNan = true; ++nanCount; };
            for (uint32_t y = 0; y < h; ++y) {
                const uint8_t* rp = src + y * mapped.RowPitch;
                for (uint32_t x = 0; x < w; ++x) {
                    if (desc.Format == DXGI_FORMAT_R32_FLOAT) {
                        uint32_t b = reinterpret_cast<const uint32_t*>(rp)[x];
                        if (((b >> 23) & 0xFFu) == 0xFFu) { if (b & 0x7FFFFFu) mark(x, y); else hasInf = true; }
                    } else if (desc.Format == DXGI_FORMAT_R16_FLOAT) {
                        uint16_t hf = reinterpret_cast<const uint16_t*>(rp)[x];
                        if (((hf >> 10) & 0x1Fu) == 0x1Fu) { if (hf & 0x3FFu) mark(x, y); else hasInf = true; }
                    } else if (desc.Format == DXGI_FORMAT_R16G16B16A16_FLOAT) {
                        const uint16_t* px = reinterpret_cast<const uint16_t*>(rp) + x * 4;
                        for (int ch = 0; ch < 4; ++ch) { uint16_t hf = px[ch]; if (((hf >> 10) & 0x1Fu) == 0x1Fu) { if (hf & 0x3FFu) mark(x, y); else hasInf = true; } }
                    } else { // R11G11B10_FLOAT: 5-bit exponents at bits 6 / 17 / 27
                        uint32_t p = reinterpret_cast<const uint32_t*>(rp)[x];
                        if (((p >> 6) & 0x1Fu) == 0x1Fu)  { if (p & 0x3Fu)          mark(x, y); else hasInf = true; }
                        if (((p >> 17) & 0x1Fu) == 0x1Fu) { if ((p >> 11) & 0x3Fu)  mark(x, y); else hasInf = true; }
                        if (((p >> 27) & 0x1Fu) == 0x1Fu) { if ((p >> 22) & 0x1Fu)  mark(x, y); else hasInf = true; }
                    }
                }
            }
            WriteTextureInspect(filepath, desc.Format, w, h, hasNan, hasInf, nanCount, fx, fy);
            if (hasNan || hasInf) g_lastDumpSawNaN = true;
        }
    }
                                  ...
                                  uint32_t frameIndex)
{
    ...
    // Bloom output (post-processing) for attribution
    auto* bloomSRV = BloomRenderer::Get().GetBloomSRV();
    if (bloomSRV) SaveSRV(dev, ctx, bloomSRV, "Bloom_output", frameIndex);

    // Final composited frame (pre-UI) — whole-frame attribution + scene-vs-final diff
    auto* finalSRV = D3D11Hook::GetPreUISceneSRV();
    if (finalSRV) SaveSRV(dev, ctx, finalSRV, "Composite_final", frameIndex);
