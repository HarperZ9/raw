...
// KINEMATIC FILTER
float4	PS_Process_Filter (VS_OUTPUT_POST IN, float4 vPos : SV_Position) : SV_Target
{
	float4 res;
	float4 coord = 0.0;
	coord.xy = IN.txcoord.xy;
	float4 original = TextureColor.Sample (SamplerColor, coord.xy);
	...
	float4 noise_big = TextureJitter.Sample(SamplerNoise, coord.xy);
	...
	float4 noise_color = TextureJitter.Sample(SamplerNoise, coord.xy);
	...
	if (enable_splitscreen == true)
	{
		...
		{
			return (IN.txcoord.x > (splitscreen_pos / 100)) ? TextureColor.Sample (SamplerColor, IN.txcoord) : res;
		}
...
// LUMASHARPEN
float4	PS_Process_LumaSharpen (VS_OUTPUT_POST IN, float4 vPos : SV_Position, float2 tex : TEXCOORD0) : SV_Target
{
	...
	half4 original = TextureColor.Sample (SamplerColor, coord.xy);
	...
	[branch] if (any (frac (original.rgb)))
	{
		...
		float4 sampleNW = float4 (tex, tex) + float4 (-px, py, -px, py) / 3 * offset_bias;
		float4 sampleSE = float4 (tex, tex) + float4 (px, -py, px, -py) / 3 * offset_bias;

		half4 blur_original = TextureColor.SampleLevel (SamplerColor, sampleNW.xy, sampleNW.w);				// North West
		blur_original += TextureColor.SampleLevel (SamplerColor, sampleSE.xy, sampleSE.w);				// South East
		...
		float4 sampleNW = float4 (tex, tex) + float4 (-px, py, -px, py) * 0.5 * offset_bias;
		float4 sampleSE = float4 (tex, tex) + float4 (px, -py, px, -py) * 0.5 * offset_bias;
		...
		if (any ((blur_original.rgb / 2) - original.rgb))
		{
			...
			float4 sampleSW = float4 (tex, tex) + float4 (-px, -py, -px, -py) * 0.5 * offset_bias;
			float4 sampleNE = float4 (tex, tex) + float4 (px, py, px, py) * 0.5 * offset_bias;

			blur_original += TextureColor.SampleLevel (SamplerColor, sampleSW.xy, sampleSW.w);			// South West
			blur_original += TextureColor.SampleLevel (SamplerColor, sampleNE.xy, sampleNE.w);			// North East
		...
		float4 sampleNNW = float4 (tex, tex) + float4 (-0.4 * px, 1.2 * py, -0.4 * px, 1.2 * py) * offset_bias;
		float4 sampleSSE = float4 (tex, tex) + float4 (0.4 * px, -1.2 * py, 0.4 * px, -1.2 * py) * offset_bias;
		float4 sampleENE = float4 (tex, tex) + float4 (1.2 * px, 0.4 * py, 0, 0) * offset_bias;
		float4 sampleWSW = float4 (tex, tex) + float4 (-1.2 * px, -0.4 * py, 0, 0) * offset_bias;

		half4 blur_original = TextureColor.SampleLevel (SamplerColor, sampleNNW.xy, sampleNNW.w);	// North North West
		blur_original += TextureColor.SampleLevel (SamplerColor, sampleSSE.xy, sampleSSE.w);		// South South East
		blur_original += TextureColor.SampleLevel (SamplerColor, sampleENE.xy, sampleENE.w);				// East North East
		blur_original += TextureColor.SampleLevel(SamplerColor, sampleWSW.xy, sampleWSW.w);			// West South West
		...
		float4 sampleNNW = float4 (tex, tex) + float4 (0.5 * -px, py, 0.5 * -px, py) * offset_bias;
		float4 sampleSSE = float4 (tex, tex) + float4 (0.5 * px, -py, 0.5 * px, -py);
		float4 sampleWSW = float4 (tex, tex) + float4 (-px, 0.5 * -py, -px, 0.5 * -py);
		float4 sampleENE = float4 (tex, tex) + float4 (px, 0.5 * py, px, 0.5 * py) * offset_bias;
		...
		half sharp = original.x - blur_original.x;
...
// BLURRING
float4	PS_Process_Blurring (VS_OUTPUT_POST IN, float4 vPos : SV_Position) : SV_Target
{
	float4 res = 0.0;
	float4 coord = 0.0;
	coord.xy = IN.txcoord.xy;
	coord.w = 0.0;
	float4 original = TextureColor.SampleLevel (SamplerColor, coord.xy, coord.w);
	...
	if (use_h_blur == false)
	{
		...
		float inv_screen_size = 1.0 / ScreenSize.x;
		...
		for (i = 0; i < 16; i ++)
		{
			...
			float4 ct = TextureColor.SampleLevel (SamplerColor, coord.xy, coord.w);
	...
	if (use_h_blur == true)
	{
		...
		for (i = 0; i < 5; i ++)
		{
			coord.x = IN.txcoord.x + offset * inv_screen_size.x * blur_range;
			//float4 ct = tex2Dlod (SamplerColor, coord);
			float4 ct = TextureColor.SampleLevel (SamplerColor, coord.xy, coord.w);
...
// SHARPENING
float4	PS_Process_Sharpening (VS_OUTPUT_POST IN, float4 vPos : SV_Position) : SV_Target
{
	float4 res;
	float4 coord = 0.0;
	coord.xy = IN.txcoord.xy;
...
// COLOR SHIFT
float4	PS_Process_ColorShift (VS_OUTPUT_POST IN, float4 vPos : SV_Position) : SV_Target
{
	float4 res;
	float4 coord = 0.0;
	coord.xy = IN.txcoord.xy;
	coord.w = 0.0;
	float4 original = TextureColor.Sample(SamplerColor, coord.xy, coord.w);
	...
	//MAKE SURE THAT THE SCREENSIZE TRUNCATION IS CORRECT
	//ASSUMING THAT ASSIGNING A VEC TO A SCALOR TAKES THE FIRST PARAM AND NOT MATCHING PARAM
	...
	coord.xy = IN.txcoord.xy;
	original = TextureColor.Sample(SamplerColor, coord.xy, coord.w);
	res.y = original.y;

	coord.xy = IN.txcoord.xy;
	coord.y -= inv_screen_size.x * color_shift_range;
	original = TextureColor.Sample(SamplerColor, coord.xy, coord.w);
	res.x = original.x;
...
// CONTRAST
float4	PS_Process_Contrast (VS_OUTPUT_POST IN, float4 vPos : SV_Position) : SV_Target
{
	float4 res;
	float3 original = TextureColor.Sample(SamplerColor, IN.txcoord.xy).rgb;
...
// VIBRANCE
float4 PS_ProcessVibrance(VS_OUTPUT_POST IN, float4 vPos : SV_Position) : SV_Target
{

	float2 VibranceDayRain; //Desaturation for rainy weather.
	
	if(WeatherAndTime.x > 3.8 && WeatherAndTime.x < 5.2)
	VibranceDayRain.x = VibranceDay / DesaturationFade + DesaturationOffset;
	else
	VibranceDayRain.x = VibranceDay;

	if(WeatherAndTime.y > 3.8 && WeatherAndTime.y < 5.2)
	VibranceDayRain.y = VibranceDay / DesaturationFade + DesaturationOffset;
	else
	VibranceDayRain.y = VibranceDay;
	
	if(WeatherAndTime.x > 13.8 && WeatherAndTime.x < 18.2)
	VibranceDayRain.x = VibranceDay / DesaturationFade + DesaturationOffset;
	else
	VibranceDayRain.x = VibranceDay;
	
	if(WeatherAndTime.y > 13.8 && WeatherAndTime.y < 18.2)
	VibranceDayRain.y = VibranceDay / DesaturationFade + DesaturationOffset;
	else
	VibranceDayRain.y = VibranceDay;
	
	if(WeatherAndTime.x > 22.8 && WeatherAndTime.x < 26.2)
	VibranceDayRain.x = VibranceDay / DesaturationFade + DesaturationOffset;
	else
	VibranceDayRain.x = VibranceDay;
	
	if(WeatherAndTime.y > 22.8 && WeatherAndTime.y < 26.2)
	VibranceDayRain.y = VibranceDay / DesaturationFade + DesaturationOffset;
	else
	VibranceDayRain.y = VibranceDay;
	...
	float4	res;
	float3	origcolor=TextureColor.SampleLevel(SamplerColor, IN.txcoord.xy, 0).rgb;
...
float4	PS_Process_AfterFX (VS_OUTPUT_POST IN, float4 vPos : SV_Position, float2 tex : TEXCOORD0) : SV_Target
{
	float4 res;
	float3 original = TextureColor.Sample (SamplerColor, IN.txcoord.xy, 0).rgb;
...
technique11 PostProcess <string UIName="Suki";>
{
	pass P0
	{

		SetVertexShader ( CompileShader (vs_5_0, VS_PostProcess()));
		SetPixelShader ( CompileShader (ps_5_0, PS_Process_Filter()));

	}
}

technique11 PostProcess1
{
	pass P0
	{
	
		SetVertexShader ( CompileShader (vs_5_0, VS_PostProcess()));
		SetPixelShader ( CompileShader (ps_5_0, PS_Process_LumaSharpen()));
...
technique11 PostProcess2
{
	pass P0
	{
		...
		SetVertexShader ( CompileShader (vs_5_0, VS_PostProcess()));
		SetPixelShader ( CompileShader (ps_5_0, PS_Process_Blurring()));

	}
}

technique11 PostProcess3
{
	pass P0
	{
	
		SetVertexShader ( CompileShader (vs_5_0, VS_PostProcess()));
		SetPixelShader ( CompileShader (ps_5_0, PS_Process_Sharpening()));
...
technique11 PostProcess4
{
	pass P0
	{
		...
		SetVertexShader ( CompileShader (vs_5_0, VS_PostProcess()));
		SetPixelShader ( CompileShader (ps_5_0, PS_Process_ColorShift()));

	}
}

technique11 PostProcess5
{
	pass P0
	{
	
		SetVertexShader ( CompileShader (vs_5_0, VS_PostProcess()));
		SetPixelShader ( CompileShader (ps_5_0, PS_Process_Contrast()));
	}
}

technique11 PostProcess6
{
	pass P0
	{
		
		SetVertexShader ( CompileShader (vs_5_0, VS_PostProcess()));
		SetPixelShader ( CompileShader (ps_5_0, PS_ProcessVibrance()));
	}
}

technique11 PostProcess7
{
	pass P0
	{
	
		SetVertexShader ( CompileShader (vs_5_0, VS_PostProcess()));
		SetPixelShader ( CompileShader (ps_5_0, PS_Process_AfterFX()));
