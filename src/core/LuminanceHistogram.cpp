...
namespace SB
{
    ...
    bool LuminanceHistogram::CompileComputeShaders(ID3D11Device* a_device)
    {
        ...
        blob = ShaderLoader::Compile("Histogram_Main", kHistogramCS, "CSMain", "cs_5_0", 0);
        if (!blob) {
        ...
        blob = ShaderLoader::Compile("Histogram_Reduction", kReductionCS, "CSReduction", "cs_5_0", 0);
        if (!blob) {
        ...
        // Compile metering CS
        blob = ShaderLoader::Compile("Histogram_Metering", kMeteringCS, "CSMetering", "cs_5_0", 0);
        if (!blob) {
    ...
    bool LuminanceHistogram::CreateResources(ID3D11Device* a_device, uint32_t w, uint32_t h)
    {
        ...
        // 256x1 R32_FLOAT texture for SRV binding to shader passes
        {
