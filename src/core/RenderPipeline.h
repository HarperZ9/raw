...
struct PassDef
{
    ...
    // OPTIONAL (Tier 1.3): range-assertion hook, called after execute() while the
    // pass output is still live. The pass supplies the SRV of what it wrote (the
    // pipeline does not track it). Null = opts out. Gated by RangeOracle::Enabled().
    std::function<void(PassContext&, const char*, ID3D11ShaderResourceView*)> post_execute;
};
...
public:
    ...
    /// Enable or disable a pass by name. Returns false if not found.
    bool SetPassEnabledByName(const char* name, bool enabled);

    /// Get all pass names + enabled state (for profiler UI).
    struct PassStatus { const char* name; bool enabled; PipelineStage stage; int priority; };
    std::vector<PassStatus> GetAllPassStatus() const;
