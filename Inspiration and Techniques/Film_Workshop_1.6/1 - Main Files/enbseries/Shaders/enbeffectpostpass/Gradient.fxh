...
// Blend middle grey value. Nothing to see here, move along.
static const float BlendMiddleGrey = 0.5;

#define BlendOverlayf(base, blend) 	(base < BlendMiddleGrey ? (2.0 * base * blend) : (1.0 - 2.0 * (1.0 - base) * (1.0 - blend)))
...
float3 GradientPass(float3 Color, float2 texcoord : TEXCOORD) : SV_Target
{
	...
	MainColor.rgb = lerp(MainColor.rgb,Color.rgb,1.0-GradientOpacity);
	...
	MainColor.rgb = GradientBlendMode(Color.rgb,MainColor.rgb);
	Color.rgb = lerp(Color.rgb,MainColor.rgb,GradientOpacity);
