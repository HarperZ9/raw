...
// FUNCTIONS /////////////////////////////////////
    ...
    float3 GrainPass(float2 position, float3 col)
    {
        ...
        noise.r = lerp(0.0,pnoise3D(float3(rotCoordsR*float2(width/grainsizeR[GRAIN_PROFILE - 1],height/grainsizeR[GRAIN_PROFILE - 1]),0.0)),grainamountR[GRAIN_PROFILE - 1]);
        noise.g = lerp(0.0,pnoise3D(float3(rotCoordsG*float2(width/grainsizeG[GRAIN_PROFILE - 1],height/grainsizeG[GRAIN_PROFILE - 1]),1.0)),grainamountG[GRAIN_PROFILE - 1]);
        noise.b = lerp(0.0,pnoise3D(float3(rotCoordsB*float2(width/grainsizeB[GRAIN_PROFILE - 1],height/grainsizeB[GRAIN_PROFILE - 1]),2.0)),grainamountB[GRAIN_PROFILE - 1]);
        ...
        noise = noise * (GRAIN_AMOUNT * 0.004);
        grey += noise;

        // Desaturate Grain if Using B&W //////////
        if(LUT_PACK == 5) grey = dot(grey, lumcoeff);
