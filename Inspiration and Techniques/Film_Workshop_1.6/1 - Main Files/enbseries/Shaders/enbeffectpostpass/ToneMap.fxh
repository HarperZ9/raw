   ...
   float4 TonemapPass( float4 colorInput)
{
  ...
  color = saturate(color - Defog2 * FogColor2); // Defog

	color *= pow(2.0f, Exposure - 1.0); // Exposure
...
float4 LensFilterPass( float4 colorInput)
{
float3 color = colorInput.rgb;

  color = saturate(color - Defog3 * FogColor3); // Defog

color *= pow(2.0f, Exposure2 - 1.0); // Exposure
