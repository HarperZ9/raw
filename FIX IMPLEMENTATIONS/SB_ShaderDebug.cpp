...
ShaderDebug::fnPresent     ShaderDebug::s_origPresent      = nullptr;
                          ...
                          IDXGISwapChain* swapChain)
{
    ...
    // ── Hook IDXGISwapChain::Present for overlay rendering ───────────
    if (swapChain)
    {
        void** vtable = *reinterpret_cast<void***>(swapChain);
        // IDXGISwapChain::Present is vtable index 8
        // (IUnknown: 3) + (IDXGIObject: 1) + (IDXGIDeviceSubObject: 1)
        // + (IDXGISwapChain: Present=8th method overall, index 8)
        constexpr UINT kVT_Present = 8;

        DWORD oldProt;
        void** entry = &vtable[kVT_Present];
        VirtualProtect(entry, sizeof(void*), PAGE_EXECUTE_READWRITE, &oldProt);
        s_origPresent = reinterpret_cast<fnPresent>(*entry);
        *entry = reinterpret_cast<void*>(HookPresent);
        VirtualProtect(entry, sizeof(void*), oldProt, &oldProt);

        SB_LOG_INFO("ShaderDebug: hooked IDXGISwapChain::Present for overlay");
    }
...
// ─── Inline hook installation (trampoline-based) ─────────────────────
// This is a simplified inline hook. For production use, replace with
// MinHook, Detours, or your preferred hooking library.
//
// The pattern: overwrite the first 14 bytes of the target function with
// a JMP to our hook, saving the original bytes + a jump-back trampoline.

template<typename FnPtr>
static void InstallInlineHook(FnPtr target, FnPtr hook, FnPtr& original)
{
    // NOTE: This is a placeholder for the actual hooking mechanism.
    // In production SkyrimBridge, replace with your hooking library:
    //
    //   MinHook:
    ...
    //     original = target;
    //     DetourAttach(reinterpret_cast<void**>(&original), hook);
    //     DetourTransactionCommit();
    //
    //   SKSE's trampoline:
    //     auto& trampoline = SKSE::GetTrampoline();
    //     original = reinterpret_cast<FnPtr>(
    //         trampoline.write_call<14>(target, hook));
    //
    // For now, we use IAT patching as a simpler alternative:
...
// ═══════════════════════════════════════════════════════════════════════════
//  Source file inference from ENB naming conventions
//
//  ENB passes source names like:
//    "enbeffect.fx"             → enbeffect.fx
//    "enbseries\\enbeffect.fx" → enbeffect.fx
//    "effect.txt"              → memory shader (ENB internal)
//    nullptr                   → unknown
//
//  We also try to match entry point names to known shader files:
//    "PS_Draw"    → likely enbeffect.fx
//    "PS_Bloom*"  → likely enbbloom.fx
// ═══════════════════════════════════════════════════════════════════════════
    ...
    const std::string& entryPoint)
{
    ...
    // Try to infer from entry point name (ENB conventions)
    std::string ep = entryPoint;
    std::transform(ep.begin(), ep.end(), ep.begin(), ::tolower);

    if (ep.find("bloom") != std::string::npos)    return "enbbloom.fx";
    if (ep.find("adapt") != std::string::npos)    return "enbadaptation.fx";
    if (ep.find("dof") != std::string::npos)      return "enbdepthoffield.fx";
    if (ep.find("lens") != std::string::npos)     return "enblens.fx";
    if (ep.find("sunsprite") != std::string::npos) return "enbsunsprite.fx";
    if (ep.find("prepass") != std::string::npos)   return "enbeffectprepass.fx";
    if (ep.find("postpass") != std::string::npos)  return "enbeffectpostpass.fx";
    if (ep.find("underwater") != std::string::npos)return "enbunderwater.fx";
    if (ep.find("water") != std::string::npos)     return "enbwater.fx";
...
// ═══════════════════════════════════════════════════════════════════════════
//  Present hook — renders overlay after the frame is composed
// ═══════════════════════════════════════════════════════════════════════════

HRESULT STDMETHODCALLTYPE ShaderDebug::HookPresent(
    IDXGISwapChain* swapChain, UINT SyncInterval, UINT Flags)
{
    ...
    if (self.m_installed)
    {
        ...
        if (self.m_overlayVisible || self.HasErrors())
        {
            if (!self.m_overlayInited)
                self.InitOverlayResources();

            if (self.m_overlayVisible)
                self.RenderOverlay();
        }
    }

    return s_origPresent(swapChain, SyncInterval, Flags);
}
