...
Complete reference for all Playground float4 parameters. Each parameter is pushed to all 9 target ENB shaders via `ENBSetParameter` every frame (dirty-tracked).
...
`UIHidden = 1` keeps parameters out of ENB's editor GUI while remaining accessible to the SDK. The `UIName` must exactly match the parameter name used by Playground's `ENBSetParameter` calls.
