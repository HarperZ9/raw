...
float3 ColorWheel(float3 color, ColorWheelStruct col) {
    ...
    color += (col.shadow * w.shadow + col.midtone * w.midtone + col.highlight * w.highlight) * 0.2;
    color += (col.lift   * w.lift   + col.gamma   * w.gamma   + col.gain      * w.gain) * 0.2;

    return color + col.offset * 0.2;
}
