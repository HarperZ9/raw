...
float4 PS_Hblur(VS_OUTPUT_POST IN) : SV_Target
{
    float2 coord    = IN.txcoord0.xy;
    float4 Original = TextureColor.Sample(Sampler1, coord);
    float Mask      = max(Original.r, max(Original.g, Original.b));
    if (SkinSmoothing==false) {return Original;}

    float4 res   = TextureColor.Sample(Sampler1, coord) * gaussianWeights[0];

    // Horizontal
    for (int i = 1; i < gaussianLoopLength; i++)
    {
        res += TextureColor.Sample(Sampler1, coord + float2(gaussianOffsets[i], 0.0) * PixelSize) * gaussianWeights[i];
        res += TextureColor.Sample(Sampler1, coord - float2(gaussianOffsets[i], 0.0) * PixelSize) * gaussianWeights[i];
    }
...
float4 PS_Vblur(VS_OUTPUT_POST IN) : SV_Target
{
    float2 coord    = IN.txcoord0.xy;
    float4 Original = TextureColor.Sample(Sampler1, coord);
    float Mask      = max(Original.r, max(Original.g, Original.b));
    ...
    for (int i = 1; i < gaussianLoopLength; i++)
    {
        res += TextureColor.Sample(Sampler1, coord + float2(0.0, gaussianOffsets[i]) * PixelSize) * gaussianWeights[i];
        res += TextureColor.Sample(Sampler1, coord - float2(0.0, gaussianOffsets[i]) * PixelSize) * gaussianWeights[i];
	}
