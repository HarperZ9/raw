...
float3 PS_FXAA(VS_OUTPUT IN) : SV_Target
{
    ...
    float3 rgbNE = TextureColor.SampleLevel(LinearSampler, uv.zw + float2(1,0)*rcpFrame.xy, 0.0).xyz;
    float3 rgbSW = TextureColor.SampleLevel(LinearSampler, uv.zw + float2(0,1)*rcpFrame.xy, 0.0).xyz;
    float3 rgbSE = TextureColor.SampleLevel(LinearSampler, uv.zw + float2(1,1)*rcpFrame.xy, 0.0).xyz;
    float3 rgbM  = TextureColor.SampleLevel(LinearSampler, uv.xy, 0.0).xyz;

    float3 luma  = float3(0.212395, 0.701049, 0.086556);
