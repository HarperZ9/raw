...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    UI_BOOL(QualityBool, "   High Quality", false)
    UI_BOOL(bADOF_ManualfocusEnable, "   Manual Focus", false)
    UI_INT(FocusMode, "      Manual Focus Mode", 1, 2, 1)
    UI_MSG(1, "         Mode 1: (Click in Scene to Focus)")
    UI_FLOAT_FINE(fADOF_ManualfocusDepth, "         Mode 2: Manual Focus Pull", 0.0, 2.0, 0.001, 0.05)
    UI_BLANK(1)
    UI_FLOAT(fApterture, "   Aperture", 0.7, 2.8, 2.4)
    UI_FLOAT_FINE(LENS_SOFTNESS, "      Bokeh Softness", 0.0, 3.0, 0.1, 1.5)
    UI_FLOAT_FINE(fADOF_ShapeRadius_set, "      Bokeh Size", 0.0, 100.0, 0.1, 10.0)
    UI_INT(fADOF_BokehIntensity_int, "      Bokeh Intensity", 0, 100, 25)
    UI_BOOL(LensType, "      Anamorphic Lens", false)
    UI_INT(AnamorphicMode, "         Anamorphic Type", 1, 2, 1)

    UI_BLANK(2)
    UI_MSG(2, "    DOF code written by Marty McFly")
    UI_MSG(3, "\xA9 2018 | Pascal Gilcher")
    ...
    #define fADOF_ShapeAnamorphRatio lerp(1.0, lerp(0.5, 0.33, AnamorphicMode), LensType)
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    float4	PS_DoF_Gauss(uniform float2 axis, uniform bool overrideAlpha, VS_OUTPUT_POST IN) : SV_Target
    {
    	...
    	float nSteps 		= floor(centerTap.a * LENS_SOFTNESS);
