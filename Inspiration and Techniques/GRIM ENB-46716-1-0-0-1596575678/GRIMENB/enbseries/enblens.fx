...
float	scale 					<string UIName="LightSprite:GlowSize ";string UIWidget="spinner";float UIMin=0.0;float UIMax=1000.0;> = {4.0};
float	brt 						<string UIName="CC: Brightness ";string UIWidget="spinner";float UIMin=0.0;float UIMax=10.0;> = {1.0};
float	contrast 				<string UIName="CC: Contrast ";string UIWidget="spinner";float UIMin=0.0;float UIMax=10.0;float UIStep=0.001;> = {1.0};
float	saturation 				<string UIName="CC: Saturation ";string UIWidget="spinner";float UIMin=0.0;float UIMax=10.0;> = {1.0};
float	gamma 					<string UIName="CC: Gamma ";string UIWidget="spinner";float UIMin=0.0;float UIMax=10.0;> = {1.0};
...
// PHOTOSHOP CC FUNCTION by ROMAIN DURA
float3 ContrastSaturationBrightness(float3 color)
{
	...
	uniform Texture2D inputtex, uniform float srcsize, uniform float destsize) : SV_Target
{
	...
	res.xyz=ContrastSaturationBrightness(res.xyz);
	if (gamma!=1.0) res.xyz=pow(res.xyz, gamma);
	res=max(res, 0.0);
	res=min(res, 16384.0);
...
float4	PS_BloomMix(VS_OUTPUT_POST IN, float4 v0 : SV_Position0) : SV_Target
{
	...
	float cbloom = dot( res.xyz, float3( 0.2125, 0.7154, 0.0721 ));
	res.xyz		 = lerp( cbloom, res.xyz, saturation );
...
technique11 MultiPassBloom <string UIName="GRIM: LIGHT.FX"; string RenderTarget="RenderTarget512";>
{
	pass p0
	{
