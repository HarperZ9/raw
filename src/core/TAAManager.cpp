...
//    2. Get backbuffer UAV (current frame color — post-pipeline)
//    3. Save CS state
...
// Tier 3.4: temporal history-buffer ping-pong witness. TAA is the only built
// ping-pong renderer (TSR/FrameGenerator are in the CMake DISABLED block);
// the host checker (bindings.py read_temporal) is renderer-general.
static void EmitTemporal(const char* buf, uint32_t frame, int readIdx, int writeIdx, bool warmup)
{
    try {
        std::error_code ec;
        std::filesystem::create_directories("Data/SKSE/Plugins/RAW/live", ec);
        std::ofstream out("Data/SKSE/Plugins/RAW/live/temporal.jsonl", std::ios::app);
        if (!out.is_open()) return;
        const char Q = static_cast<char>(34);
        out << '{' << Q << "frame" << Q << ':' << frame
            << ',' << Q << "buffer" << Q << ':' << Q << buf << Q
            << ',' << Q << "read_idx" << Q << ':' << readIdx
            << ',' << Q << "write_idx" << Q << ':' << writeIdx
            << ',' << Q << "warmup" << Q << ':' << (warmup ? "true" : "false")
            << '}' << static_cast<char>(10);
    } catch (...) {}
}
...
bool TAAManager::CompileShader(ID3D11Device* dev)
{
    ...
    ID3DBlob* blob = ShaderLoader::Compile("TAA_Resolve", kResolveCS, "main", "cs_5_0", flags);
    if (!blob) {
...
void TAAManager::Resolve(ID3D11DeviceContext* ctx, IDXGISwapChain* swapChain)
{
    ...
    if (m_firstFrame) {
        ...
        backbuffer->Release();
        ++m_frameIndex;
        EmitTemporal("taa.history", m_frameIndex, 0, 0, true);
        return;
    }
    ...
    // ── Swap ping-pong ───────────────────────────────────────────────
    EmitTemporal("taa.history", m_frameIndex, 1 - m_writeIdx, m_writeIdx, false);
...
ID3D11ShaderResourceView* TAAManager::GetHistorySRV() const
{
    ...
    // For shader passes sampling during the NEXT frame, they want
