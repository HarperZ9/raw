...
bool DebugRenderer::CreateShaders()
{
    ...
    HRESULT hr = D3DCompile(kDebugVS, sizeof(kDebugVS) - 1,
                            "SB_DebugVS", nullptr, nullptr,
                            "main", "vs_5_0", flags, 0,
                            &vsBlob, &err);
    if (FAILED(hr)) {
        if (err) {
            SKSE::log::error("DebugRenderer: VS compile error: {}",
    ...
    err = nullptr;
    hr = D3DCompile(kDebugPS, sizeof(kDebugPS) - 1,
                    "SB_DebugPS", nullptr, nullptr,
                    "main", "ps_5_0", flags, 0,
                    &psBlob, &err);
    if (FAILED(hr)) {
        if (err) {
            SKSE::log::error("DebugRenderer: PS compile error: {}",
                             static_cast<const char*>(err->GetBufferPointer()));
            err->Release();
        }
        return false;
