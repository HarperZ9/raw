...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    bool   QualityBool < string UIName="   High Quality";> = {false};
    bool   bADOF_ManualfocusEnable < string UIName="   Manual Focus";> = {false};
    int  FocusMode <string UIName="      Manual Focus Mode"; string UIWidget="spinner"; float UIMin=1; float UIMax=2;> = {1};
    float	fADOF_ManualfocusDepth		< string UIName="         Manual Focus Pull (Mode 2)";		string UIWidget="Spinner";	float UIStep=0.001;	float UIMin=0.00;	float UIMax=1.0;	> = {0.05};
    int  Empty_Row0 <string UIName=" "; string UIWidget="spinner"; float UIMin=0; float UIMax=0;> = {0};
    float  fApterture <string UIName="Aperture"; string UIWidget="spinner"; float UIMin=0.7; float UIMax=2.8;> = {2.4};
    float	fADOF_ShapeRadius_set		< string UIName="   Bokeh Size";		string UIWidget="Spinner";	float UIStep=0.1;	float UIMin=0.0;	float UIMax=100.0;	> = {20.0};
    int	fADOF_BokehIntensity_int		< string UIName="   Bokeh Intensity";			string UIWidget="Spinner";	float UIMin=0;	float UIMax=100;	> = {50};
    bool   LensType < string UIName="   Anamorphic Lens";> = {false};
    ...
    #define fADOF_NearBlurCurve lerp(10.0, lerp(0.6, 2.7, (fApterture * 0.357)), bADOF_ManualfocusEnable)
    ...
    #define fADOF_ShapeRadius lerp((fADOF_ShapeRadius_set * lerp(1.25, 1.5, LensType)), (fADOF_ShapeRadius_set * lerp(0.75, 1.0, LensType)), (fApterture * 0.357))
    #define iADOF_ShapeVertices 8
    #define iADOF_ShapeQuality lerp(3, 5, QualityBool)
    #define fADOF_ShapeCurvatureAmount lerp(0.15, 1.0, LensType)
    #define fADOF_ShapeRotation 66
    #define fADOF_ShapeAnamorphRatio lerp(1.0, 0.5, LensType)
    #define fADOF_SmootheningAmount 0

    #if (ADOF_OPTICAL_VIGNETTE_ENABLE != 0)
     #define fADOF_ShapeVignetteCurve 0.5
     #define fADOF_ShapeVignetteAmount 1.0
    #endif

    #if (ADOF_CHROMATIC_ABERRATION_ENABLE != 0)
     #define fADOF_ShapeChromaAmount -0.5
     #define iADOF_ShapeChromaMode 0
    #endif

    int Empty_Row1     <string UIName="   "; string UIWidget="spinner"; float UIMin=0; float UIMax=0;> = {0};
    int Empty_Row2     <string UIName="    DOF code written by Marty McFly"; string UIWidget="spinner"; float UIMin=0; float UIMax=0;> = {0};
    int Empty_Row3     <string UIName="\xA9 2018 | Pascal Gilcher"; string UIWidget="spinner"; float UIMin=9001; float UIMax=9001;> = {9001};
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    #define FPS_HAND_BLUR_CUTOFF_DIST		lerp(0.3353, 0.0, bADOF_ManualfocusEnable)		//fps hand depth (x10.000), change if you perceive blurred fps weapons.
    #define FPS_HAND_BLUR_CUTOFF_CHECK		0		//blur = max if depth > hand depth, else 0, useful for tweaking above param
    #define GAUSSIAN_BUILDUP_MULT			1.0		//value of x -> gaussian reaches max radius at |CoC| == 1/x
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    SAMPLER(SamplerPoint, POINT, Clamp)
    SAMPLER(SamplerLinear, LINEAR, Clamp)
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    float4	PS_ReadFocus(VS_OUTPUT_POST IN) : SV_Target {
    	float scenefocus = 0.0;
            ...
            [branch]
    	if(bADOF_ManualfocusEnable == false) {
    		float samples = 10.0;
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    float4	PS_Focus(VS_OUTPUT_POST IN) : SV_Target {
        ...
        return (FocusMode < 2) ? 0.00390625 * lerp(prevFocus, currFocus, DofParameters.w * (1 + 0.75*bADOF_ManualfocusEnable)) : currFocus * 0.00390625;
    }
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    TECHNIQUE(ReadFocus,
        PASS(p0, VS_Quad, PS_ReadFocus)
    )

    TECHNIQUE(Focus,
        PASS(p0, VS_Quad, PS_Focus)
    )

    TECHNIQUE_UI(DOF, "Advanced DOF",
        PASS(p0, VS_Quad, PS_CoC)
    )

    TECHNIQUE(DOF1,
        PASS_ARGS_VS(p0, VS_DoF, (fADOF_RenderResolutionMult), PS_DoF_Main)
    )

    TECHNIQUE(DOF2,
        PASS(p0, VS_Quad, PS_DoF_Combine)
    )

    TECHNIQUE(DOF3,
        PASS_ARGS_PS(p0, VS_Quad, PS_DoF_Gauss, (float2(0,1),0))
    )

    TECHNIQUE(DOF4,
        PASS_ARGS_PS(p0, VS_Quad, PS_DoF_Gauss, (float2(1,0),1))
    )

    #if (ADOF_CHROMATIC_ABERRATION_ENABLE != 0)
        TECHNIQUE(DOF5,
            PASS(p0, VS_Quad, PS_DoF_ChromaticAberration)
        )
    #endif
