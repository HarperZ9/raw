...
How to use SkyrimBridge data in ENB shaders.
...
SkyrimBridge may not be installed. All extern float4s default to zero, so guard features:

```hlsl
// Check if feedback data is available
bool SB_HasFeedback() { return SB_Computed_Scene.w > 0.5; }

// Check if SkyrimBridge is active (any non-zero data)
