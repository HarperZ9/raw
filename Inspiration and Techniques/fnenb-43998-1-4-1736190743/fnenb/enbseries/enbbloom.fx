...
float3	EBlueShiftColor
<
	string UIName = "BlueShift color";
	string UIWidget = "color";
> = {0.6, 0.5, 1.0};

float	EBlueShiftAmount
<
	string UIName="BlueShift Amount";
	string UIWidget="Spinner";
	float UIMin=0.0;
	float UIMax=100.0;
> = {1.0};
...
float3	FuncBlur(Texture2D inputtex, float2 uvsrc, float srcsize, float destsize)
{
	...
	//+++ blue shift
	float3	violet = EBlueShiftColor / max(dot(EBlueShiftColor, 0.333), 0.000001);
	//this applies on dark and when contrast
	float3	srcbloom = inputtex.Sample(Sampler1, uvsrc.xy).xyz;
	float	diff = dot(curr.xyz, 0.333) - dot(srcbloom.xyz, 0.333);
	diff *= saturate(1.0 - srcsize * (1.0/1024.0)); //ignore highest resolution bloom
	diff = max(diff, 0.0);
	float	gray = EBlueShiftAmount * diff;//max(srcbloom.x, max(srcbloom.y, srcbloom.z));
	float	mixfact = gray/(1.0 + gray);
	curr.xyz *= lerp(1.0, violet.xyz, mixfact);
