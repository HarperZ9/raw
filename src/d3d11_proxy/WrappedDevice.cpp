...
// --- Tier 3.6: create-side resource (VRAM) logging helpers (gated, default OFF) ---
static uint32_t RAW_BppForFormat(DXGI_FORMAT f)
{
    switch (f) {
    case DXGI_FORMAT_R32G32B32A32_FLOAT: case DXGI_FORMAT_R32G32B32A32_UINT: return 16;
    ...
    case DXGI_FORMAT_R32_FLOAT: case DXGI_FORMAT_D32_FLOAT:
    case DXGI_FORMAT_R24G8_TYPELESS: case DXGI_FORMAT_D24_UNORM_S8_UINT: return 4;
    case DXGI_FORMAT_R16_FLOAT: case DXGI_FORMAT_R16_UNORM:
    case DXGI_FORMAT_D16_UNORM: case DXGI_FORMAT_R8G8_UNORM: return 2;
    case DXGI_FORMAT_R8_UNORM: return 1;
    default: return 4;
    }
}
static uint64_t RAW_EstimateTex2DBytes(const D3D11_TEXTURE2D_DESC& d)
{
    uint32_t mips = d.MipLevels;
    if (mips == 0) { uint32_t m = 1, s = (d.Width > d.Height ? d.Width : d.Height); while (s > 1) { s >>= 1; ++m; } mips = m; }
    uint64_t bpp = RAW_BppForFormat(d.Format), total = 0;
    for (uint32_t m = 0; m < mips; ++m) {
        uint32_t ww = d.Width >> m; if (!ww) ww = 1;
        uint32_t hh = d.Height >> m; if (!hh) hh = 1;
        total += (uint64_t)ww * hh * bpp;
    }
    return total * (d.ArraySize ? d.ArraySize : 1) * (d.SampleDesc.Count ? d.SampleDesc.Count : 1);
}
static void RAW_EmitResourceCreate(const char* type, const void* ptr, uint64_t size)
{
    try {
        std::error_code ec;
        std::filesystem::create_directories("Data/SKSE/Plugins/RAW/live", ec);
        std::ofstream out("Data/SKSE/Plugins/RAW/live/resources.jsonl", std::ios::app);
        if (!out.is_open()) return;
        const char Q = static_cast<char>(34);
        out << '{' << Q << "op" << Q << ':' << Q << "create" << Q
            << ',' << Q << "type" << Q << ':' << Q << type << Q
    ...
    ID3D11Texture2D** ppTexture2D)
{
    ...
    if (PG_IsResourceLog() && SUCCEEDED(hr) && pDesc && ppTexture2D && *ppTexture2D)
        RAW_EmitResourceCreate("texture2d", *ppTexture2D, RAW_EstimateTex2DBytes(*pDesc));
    return hr;
}
    ...
    ID3D11Buffer** ppBuffer)
{
    ...
    if (PG_IsResourceLog() && SUCCEEDED(hr) && pDesc && ppBuffer && *ppBuffer)
        RAW_EmitResourceCreate("buffer", *ppBuffer, (uint64_t)pDesc->ByteWidth);
    return hr;
}
