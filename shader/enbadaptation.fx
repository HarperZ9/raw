...
float4 PS_Histogram() : SV_Target
{
    ...
    float2 adaptAnchor = 32.0;
    float2 accumulate  = float2(HighPercent - 1.0, LowPercent - 1.0) * 256.0;

    [loop] for(int l = 15; l >= 0; l--)
    {
        accumulate += bin[l].w;
        adaptAnchor = (accumulate.xy >= 0.0 && accumulate.xy < max(bin[l].ww, 0.0001))
                    ? l * 4.0 + accumulate.xy / max(bin[l].ww, 0.0001) + 3.0 : adaptAnchor;

        accumulate += bin[l].z;
        adaptAnchor = (accumulate.xy >= 0.0 && accumulate.xy < max(bin[l].zz, 0.0001))
                    ? l * 4.0 + accumulate.xy / max(bin[l].zz, 0.0001) + 2.0 : adaptAnchor;

        accumulate += bin[l].y;
        adaptAnchor = (accumulate.xy >= 0.0 && accumulate.xy < max(bin[l].yy, 0.0001))
                    ? l * 4.0 + accumulate.xy / max(bin[l].yy, 0.0001) + 1.0 : adaptAnchor;

        accumulate += bin[l].x;
        adaptAnchor = (accumulate.xy >= 0.0 && accumulate.xy < max(bin[l].xx, 0.0001))
                    ? l * 4.0 + accumulate.xy / max(bin[l].xx, 0.0001) + 0.0 : adaptAnchor;
    }
    ...
    // NaN safety — if any computation produced NaN, use previous frame's value
    adapted = (isnan(adapted) || isinf(adapted)) ? prevAdapt : adapted;
