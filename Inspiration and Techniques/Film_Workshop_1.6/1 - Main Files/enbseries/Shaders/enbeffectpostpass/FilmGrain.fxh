...
float4 rnm(in float2 tc)
  ...
  float pnoise3D(in float3 p)
  {
    ...
    #if(CUSTOM_MODE == 0)
    float3 GrainPassNegative(float2 position, float3 col)
    {
    ...
    float2 rot = rotCoordsR*float2(width/grainsizenegative,height/grainsizenegative);
    ...
    float3 noise = float3(pNoise, pNoise, pNoise);
        ...
        noise.g = lerp(noise.r,pnoise3D(float3(rotCoordsG*float2(width/grainsizenegative,height/grainsizenegative),1.0)),coloramountnegative);
        noise.b = lerp(noise.r,pnoise3D(float3(rotCoordsB*float2(width/grainsizenegative,height/grainsizenegative),2.0)),coloramountnegative);

    //noisiness response curve based on scene luminance
    float3 lumcoeff = float3(0.299,0.587,0.114);
    float luminance = lerp(0.0,dot(col, lumcoeff),lumamountnegative);
...
float3 GrainPassPrint(float2 position, float3 col)
{
...
float3 rotOffset = float3(1.425,3.892,5.835); //rotation offset values
float2 rotCoordsR = coordRot(position, Timer.x*16777216 + rotOffset.x);
float2 rot = rotCoordsR*float2(width/grainsizeprint,height/grainsizeprint);
float pNoise = pnoise3D(float3(rot.x,rot.y,0.0));
float3 noise = float3(pNoise, pNoise, pNoise);

    float2 rotCoordsG = coordRot(position, Timer.x*16777216 + rotOffset.y);
    float2 rotCoordsB = coordRot(position, Timer.x*16777216 + rotOffset.z);
    noise.g = lerp(noise.r,pnoise3D(float3(rotCoordsG*float2(width/grainsizeprint,height/grainsizeprint),1.0)),coloramountprint);
    noise.b = lerp(noise.r,pnoise3D(float3(rotCoordsB*float2(width/grainsizeprint,height/grainsizeprint),2.0)),coloramountprint);

//noisiness response curve based on scene luminance
float3 lumcoeff = float3(0.299,0.587,0.114);
float luminance = lerp(0.0,dot(col, lumcoeff),lumamountprint);
float lum = smoothstep(0.2,0.0,luminance);
lum += luminance;
