...
// Coherence membrane (Tier 3.3): DXBC register reflection. At the post-compile blob,
// read which registers the shader actually declares, so the host can flag a
// slot->register mismatch (shader reads register(t15), injector binds t27). D3DReflect
// is reached via GetProcAddress on the already-loaded d3dcompiler (no new link).
static std::string JsonEsc(const std::string& in);
static void WriteShaderVariant(HRESULT hr, const char* src, SIZE_T srcSize,  // Tier 3.2; defined below
                               const D3D_SHADER_MACRO* defs, const char* sourceName,
                               const char* entry, const char* target);  // defined below

typedef HRESULT (WINAPI* PFN_D3DReflect)(LPCVOID, SIZE_T, REFIID, void**);

static PFN_D3DReflect GetReflectFn()
{
    static PFN_D3DReflect fn = nullptr;
    static bool tried = false;
    if (!tried) {
        tried = true;
        HMODULE m = GetModuleHandleA("d3dcompiler_47.dll");
        if (!m) m = GetModuleHandleA("d3dcompiler_46.dll");
        if (m) fn = reinterpret_cast<PFN_D3DReflect>(GetProcAddress(m, "D3DReflect"));
    }
    return fn;
}

static void ReflectShaderBindings(const char* src, const char* entry, const char* target, ID3DBlob* code)
{
    if (!code) return;
    auto reflect = GetReflectFn();
    if (!reflect) return;
    ID3D11ShaderReflection* r = nullptr;
    if (FAILED(reflect(code->GetBufferPointer(), code->GetBufferSize(),
                       __uuidof(ID3D11ShaderReflection), reinterpret_cast<void**>(&r))) || !r)
        return;
    D3D11_SHADER_DESC sd;
    if (FAILED(r->GetDesc(&sd))) { r->Release(); return; }
    try {
        std::error_code ec;
        std::filesystem::create_directories("Data/SKSE/Plugins/RAW/live", ec);
        std::ofstream out("Data/SKSE/Plugins/RAW/live/shader_reflection.jsonl", std::ios::app);
        if (out.is_open()) {
            const char Q = static_cast<char>(34);
            for (UINT i = 0; i < sd.BoundResources; ++i) {
                D3D11_SHADER_INPUT_BIND_DESC bd;
                if (FAILED(r->GetResourceBindingDesc(i, &bd))) continue;
                out << '{' << Q << "shader" << Q << ':' << Q << JsonEsc(src ? src : "") << Q
                    << ',' << Q << "entry" << Q << ':' << Q << JsonEsc(entry ? entry : "") << Q
                    << ',' << Q << "profile" << Q << ':' << Q << JsonEsc(target ? target : "") << Q
                    << ',' << Q << "name" << Q << ':' << Q << JsonEsc(bd.Name ? bd.Name : "") << Q
                    << ',' << Q << "type" << Q << ':' << static_cast<int>(bd.Type)
                    << ',' << Q << "reg" << Q << ':' << bd.BindPoint
                    << ',' << Q << "count" << Q << ':' << bd.BindCount
                    << '}' << static_cast<char>(10);
            }
        }
    } catch (...) {}
    ...
    ID3DBlob** ppCode, ID3DBlob** ppErrorMsgs)
{
    ...
    // Coherence membrane (Tier 3.2): #define-permutation witness (defines live ONLY here).
    WriteShaderVariant(hr, compileSource, compileSize, finalDefines,
                       pSourceName, pEntrypoint, pTarget);
    ...
    if (SUCCEEDED(hr) && ppCode && *ppCode)
        ReflectShaderBindings(pSourceName, pEntrypoint, pTarget, *ppCode);
...
// ── Coherence membrane (Tier 0.2): machine-readable compile witness ──────────
static std::string JsonEsc(const std::string& in)
{
...
// --- Tier 3.2: shader #define-permutation witness ---------------------------
// Defines exist ONLY at the D3DCompile hooks (the DXBC blob has them erased), so
// they are captured here, never reflected from the binary. The "hash" is a
// deterministic permutation id (FNV-1a over source+defines+entry+target); it is
// NOT the bytecode ps_hash used at bind time (different domain) -- correlate
// variant <-> shader_used by file/entry, not by hash.
static uint64_t SV_FnvHash(const char* src, SIZE_T srcSize, const char* defStr,
                           const char* entry, const char* target)
{
    constexpr uint64_t OFF = 14695981039346656037ULL, PR = 1099511628211ULL;
    uint64_t h = OFF;
    auto feed = [&](const char* p, size_t n) { for (size_t i = 0; i < n; ++i) { h ^= (unsigned char)p[i]; h *= PR; } };
    if (src) feed(src, srcSize);
    if (defStr) feed(defStr, std::strlen(defStr));   // canonical: sorted define string (audit E)
    if (entry) feed(entry, std::strlen(entry));
    if (target) feed(target, std::strlen(target));
    return h;
}
static void WriteShaderVariant(HRESULT hr, const char* src, SIZE_T srcSize,
                               const D3D_SHADER_MACRO* defs, const char* sourceName,
                               const char* entry, const char* target)
{
    try {
        std::error_code ec;
        std::filesystem::create_directories("Data/SKSE/Plugins/RAW/live", ec);
        std::ofstream f("Data/SKSE/Plugins/RAW/live/shader_variants.jsonl", std::ios::app);
        if (!f.is_open()) return;
        std::vector<std::string> kv;
        if (defs) for (auto* d = defs; d->Name; ++d) {
            std::string one = d->Name; one += '='; if (d->Definition) one += d->Definition;
            kv.push_back(std::move(one));
        }
        std::sort(kv.begin(), kv.end());
        std::string defStr; for (size_t i = 0; i < kv.size(); ++i) { if (i) defStr += '|'; defStr += kv[i]; }
        uint64_t hash = SV_FnvHash(src, srcSize, defStr.c_str(), entry, target);
        const char Q = static_cast<char>(34);
        f << '{' << Q << "hash" << Q << ':' << Q << "0x" << std::hex << hash << std::dec << Q
          << ',' << Q << "file" << Q << ':' << Q << JsonEsc(sourceName ? sourceName : "<memory>") << Q
          << ',' << Q << "entry" << Q << ':' << Q << JsonEsc(entry ? entry : "<unknown>") << Q
          << ',' << Q << "profile" << Q << ':' << Q << JsonEsc(target ? target : "?") << Q
          << ',' << Q << "defines" << Q << ':' << Q << JsonEsc(defStr) << Q
          << ',' << Q << "ndefs" << Q << ':' << kv.size()
          << ',' << Q << "succeeded" << Q << ':' << (SUCCEEDED(hr) ? "true" : "false")
          << '}' << static_cast<char>(10);
    } catch (...) {}
}

static void WriteCompileJson(const CompilationAttempt& a, const std::filesystem::path& logPath)
{
    try {
        auto dir = logPath.parent_path() / "live";
        std::error_code ec; std::filesystem::create_directories(dir, ec);
        std::ofstream f(dir / "compile_errors.jsonl", std::ios::app);
        if (!f.is_open()) return;
        auto ts = std::chrono::duration_cast<std::chrono::seconds>(
                      a.timestamp.time_since_epoch()).count();
        f << "{\"ts\":" << ts
          << ",\"file\":\"" << JsonEsc(a.sourceFile) << "\""
          << ",\"entry\":\"" << JsonEsc(a.entryPoint) << "\""
          << ",\"profile\":\"" << JsonEsc(a.profile) << "\""
          << ",\"succeeded\":" << (a.succeeded ? "true" : "false")
          << ",\"compile_ms\":" << a.compileTimeMs
          << ",\"errors\":[";
        for (size_t i = 0; i < a.errors.size(); ++i) {
            const auto& e = a.errors[i];
            if (i) f << ",";
            f << "{\"line\":" << e.line << ",\"col\":" << e.column
              << ",\"code\":\"" << JsonEsc(e.errorCode) << "\""
              << ",\"sev\":" << static_cast<int>(e.severity)
              << ",\"msg\":\"" << JsonEsc(e.message) << "\"}";
        }
        f << "]}\n";
    } catch (...) { /* never disturb compilation */ }
}
    ...
    ID3DBlob** ppErrorMsgs, double elapsedMs)
{
    ...
    if (m_config.persistLog)
        WriteCompileJson(attempt, m_logPath);
