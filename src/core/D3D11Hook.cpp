...
namespace D3D11Hook
{
    ...
    void LedgerRegisterName(void* res, const char* name) { if (s_proxy && s_proxy->RegisterResourceName && res) s_proxy->RegisterResourceName(res, name); }
    void LedgerSetPassMarker(const char* name)           { if (s_proxy && s_proxy->SetPassMarker) s_proxy->SetPassMarker(name); }
