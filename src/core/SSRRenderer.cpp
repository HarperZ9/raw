...
//    5. Register SRV at t21 for ENB shaders
//=============================================================================
...
// Hi-Z accelerated screen-space ray march (Stachowiak 2015).
//
// For each half-res pixel, reflect the view direction across the surface
// normal, then march the reflected ray through the Hi-Z pyramid.
// Start at the coarsest useful mip, advance until the ray goes behind
// a depth surface, then refine by stepping down one mip level.
//
...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    for (int i = 0; i < MaxSteps; i++)
    {
        ...
        // Sample linear depth at full-res screen coordinate
...
// Resolve — sample scene color at ray hit UV, apply Fresnel + fades.
...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    // If current frame has a hit, blend normally
    // If current frame has no hit but history does, fade history slowly
    float alpha = TemporalAlpha;

    if (confidenceCurrent < 0.001)
    {
        // No hit this frame — slowly fade history
        alpha = 0.02;
    }
...
bool SSRRenderer::Initialize(ID3D11Device* dev, ID3D11DeviceContext* ctx, IDXGISwapChain* sc)
{
    ...
    // Register as PreENB pipeline pass (reflections visible to ENB shaders)
...
bool SSRRenderer::CompileShaders()
{
    ...
    if (!CompileCS("SSRRayMarch",  kRayMarchCS, &m_rayMarchCS)) return false;
    if (!CompileCS("SSRResolve",   kResolveCS,  &m_resolveCS))  return false;
    if (!CompileCS("SSRTemporal",  kTemporalCS, &m_temporalCS)) return false;
...
bool SSRRenderer::CreateResources()
{
    ...
    // Helper: create a half-res R16G16B16A16_FLOAT texture with SRV + UAV
    auto CreateHalfTex = [&](const char* name,
                              ...
                              ID3D11UnorderedAccessView** outUAV) -> bool
    {
        ...
        if (FAILED(hr)) {
            SKSE::log::error("SSRRenderer: failed to create {} (0x{:X})",
                             name, static_cast<uint32_t>(hr));
            return false;
        }
    ...
    if (!CreateHalfTex("hitBuffer", &m_hitBuffer, &m_hitBufferSRV, &m_hitBufferUAV))
        return false;

    // Pass 2 output: resolved reflection color
    if (!CreateHalfTex("reflRaw", &m_reflRaw, &m_reflRawSRV, &m_reflRawUAV))
        return false;

    // Pass 3: ping-pong history
    if (!CreateHalfTex("reflHistory0", &m_reflHistory[0], &m_reflHistorySRV[0], &m_reflHistoryUAV[0]))
        return false;
    if (!CreateHalfTex("reflHistory1", &m_reflHistory[1], &m_reflHistorySRV[1], &m_reflHistoryUAV[1]))
        return false;
    ...
    // Constant buffers
    {
        ...
        desc.ByteWidth = sizeof(RayMarchCBData);
        hr = m_device->CreateBuffer(&desc, nullptr, &m_rayMarchCB);
        if (FAILED(hr)) return false;

        desc.ByteWidth = sizeof(ResolveCBData);
        hr = m_device->CreateBuffer(&desc, nullptr, &m_resolveCB);
        if (FAILED(hr)) return false;
...
ID3D11ShaderResourceView* SSRRenderer::GetReflectionSRV() const
{
    ...
    return m_reflHistorySRV[1 - m_pingPongIdx];
}
...
void SSRRenderer::ExecutePass(PassContext& ctx)
{
    ...
    // ═════════════════════════════════════════════════════════════════════
    {
        ...
        if (SUCCEEDED(ctx.context->Map(m_rayMarchCB, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped))) {
            std::memcpy(mapped.pData, &cb, sizeof(cb));
            ctx.context->Unmap(m_rayMarchCB, 0);
        }
    ...
    // ═════════════════════════════════════════════════════════════════════
    {
        ...
        D3D11_MAPPED_SUBRESOURCE mapped;
        if (SUCCEEDED(ctx.context->Map(m_resolveCB, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped))) {
            std::memcpy(mapped.pData, &cb, sizeof(cb));
            ctx.context->Unmap(m_resolveCB, 0);
        }
    ...
    // ═════════════════════════════════════════════════════════════════════
    {
        ...
        ID3D11ShaderResourceView* srvs[] = { m_reflRawSRV, m_reflHistorySRV[readIdx] };
        ctx.context->CSSetShaderResources(0, 2, srvs);
