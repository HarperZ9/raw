...
void SceneMatrices::Update(const AllData& data)
{
    ...
    // Derive aspect ratio from swap chain (cam.Params.w is unreliable — often 1.0)
    m_aspect = 1.777f;
    if (auto* sc = D3D11Hook::GetSwapChain()) {
        DXGI_SWAP_CHAIN_DESC scDesc;
        if (SUCCEEDED(sc->GetDesc(&scDesc)) && scDesc.BufferDesc.Height > 0) {
            m_aspect = static_cast<float>(scDesc.BufferDesc.Width) /
...
void SceneMatrices::UpdateFromNiCamera()
{
    ...
    // NiCamera port is normalized [0,1] — cannot derive aspect from it.
    // Use the actual render resolution from the swap chain instead.
    float aspect = 1.777f; // default 16:9
    if (auto* sc = D3D11Hook::GetSwapChain()) {
        DXGI_SWAP_CHAIN_DESC scDesc;
        if (SUCCEEDED(sc->GetDesc(&scDesc)) && scDesc.BufferDesc.Height > 0) {
            aspect = static_cast<float>(scDesc.BufferDesc.Width) /
                     static_cast<float>(scDesc.BufferDesc.Height);
        }
    }
