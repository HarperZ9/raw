...
// Linearize reversed-Z depth into view-space Z (R32_FLOAT).
// Input: reversed-Z from HiZ CSCopy (near=1, far=0).
// Reversed-Z formula: viewZ = N*F / (N + z*(F-N))
...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    // Reversed-Z: z=1 near, z=0 far/sky
...
bool SharedGPUResources::CompileLinearizeCS(ID3D11Device* dev)
{
    ...
    ID3DBlob* blob = ShaderLoader::Compile("Shared_LinearizeDepth", kLinearizeDepthCS, "main", "cs_5_0", flags);
    if (!blob) {
