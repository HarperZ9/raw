...
namespace SB
{
    ...
    namespace
    {
            ...
            UINT                        a_numInstances)
        {
            ...
            RenderInspector::Get().OnPSSetShader(a_shader);
            ...
            INT  a_baseVertex)
        {
            ...
            RenderInspector::Get().OnDrawIndexed(a_ctx, a_indexCount, a_startIndex, a_baseVertex);
    ...
    void DXBCPatcher::OnBeforeDraw(ID3D11DeviceContext* a_ctx)
    {
        ...
        // Only act during BSLightingShader passes
        if (!detail::g_inLightingPass || !m_currentPS)
            return;

        // Look up or create patched shader
        auto it = m_patchCache.find(m_currentPS);
        if (it == m_patchCache.end()) {
            // First encounter — try to patch
            ID3D11PixelShader* patched = PatchShader(m_device, m_currentPS);
            m_patchCache[m_currentPS] = patched;  // nullptr if failed
            it = m_patchCache.find(m_currentPS);
        }

        ID3D11PixelShader* patchedPS = it->second;
        if (!patchedPS)
            return;  // Patching failed for this shader

        // Swap to patched PS
        s_origPSSetShader(a_ctx, patchedPS, nullptr, 0);
        m_swappedThisDraw = true;

        // Bind the material ID UAV at u4
        GBufferManager::Get().BindUAV(a_ctx, 4);

        // Update and bind the material type CB at b15
        uint8_t matType = static_cast<uint8_t>(detail::g_currentMaterial);
        UpdateMaterialCB(a_ctx, matType);
    }
    ...
    void DXBCPatcher::OnAfterDraw(ID3D11DeviceContext* a_ctx)
    {
        ...
        // Unbind UAV
        GBufferManager::Get().UnbindUAV(a_ctx, 4);
