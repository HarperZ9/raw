...
#define DIAGNOSTIC_PASSTHROUGH  0
...
// SkyrimBridge: Inline only the params needed by CAS + WeatherLUT.
// DO NOT #include the full SkyrimBridge.fxh here — it adds 102 float4 params
// which causes constant buffer overflow when combined with ~310 UI params
// from the SHADERGROUP includes above.
#define SKYRIMBRIDGE_FXH 1  // Prevent accidental inclusion by sub-headers

float4 SB_Render_Frame;        // x=frameCount — used for SB_IsActive()
float4 SB_Weather_Flags;       // y=cloudy, z=rain, w=snow
float4 SB_Weather_Transition;  // x=transition blend [0,1]
float4 SB_Lightning;           // x=thunderTimer, z=flashIntensity
float4 SB_Fog_Density;         // x=nearDist, y=density
float4 SB_Precipitation;       // x=type, y=intensity
float4 SB_Time;                // x=gameHour, y=sunriseHour, z=sunsetHour, w=dayProgress

// Inline helpers matching SkyrimBridge.fxh API
bool SB_IsActive() { return SB_Render_Frame.x > 0.0; }

// Effect modules
#include "Helper/Effect_CAS.fxh"
#include "Helper/Effect_WeatherLUT.fxh"
...
// --- Weather LUT Textures (7 weather x 3 time = 21, using identity LUT as placeholder) ---
// Clear
Texture2D TexWLUT_Clear_Day        <string ResourceName="Textures/LUTs/Weather/Clear_Day.png";>;
Texture2D TexWLUT_Clear_DawnDusk   <string ResourceName="Textures/LUTs/Weather/Clear_DawnDusk.png";>;
Texture2D TexWLUT_Clear_Night      <string ResourceName="Textures/LUTs/Weather/Clear_Night.png";>;
// Cloudy
Texture2D TexWLUT_Cloudy_Day       <string ResourceName="Textures/LUTs/Weather/Cloudy_Day.png";>;
Texture2D TexWLUT_Cloudy_DawnDusk  <string ResourceName="Textures/LUTs/Weather/Cloudy_DawnDusk.png";>;
Texture2D TexWLUT_Cloudy_Night     <string ResourceName="Textures/LUTs/Weather/Cloudy_Night.png";>;
// Foggy
Texture2D TexWLUT_Foggy_Day        <string ResourceName="Textures/LUTs/Weather/Foggy_Day.png";>;
Texture2D TexWLUT_Foggy_DawnDusk   <string ResourceName="Textures/LUTs/Weather/Foggy_DawnDusk.png";>;
Texture2D TexWLUT_Foggy_Night      <string ResourceName="Textures/LUTs/Weather/Foggy_Night.png";>;
// Rain
Texture2D TexWLUT_Rain_Day         <string ResourceName="Textures/LUTs/Weather/Rain_Day.png";>;
Texture2D TexWLUT_Rain_DawnDusk    <string ResourceName="Textures/LUTs/Weather/Rain_DawnDusk.png";>;
Texture2D TexWLUT_Rain_Night       <string ResourceName="Textures/LUTs/Weather/Rain_Night.png";>;
// Thunder
Texture2D TexWLUT_Thunder_Day      <string ResourceName="Textures/LUTs/Weather/Thunder_Day.png";>;
Texture2D TexWLUT_Thunder_DawnDusk <string ResourceName="Textures/LUTs/Weather/Thunder_DawnDusk.png";>;
Texture2D TexWLUT_Thunder_Night    <string ResourceName="Textures/LUTs/Weather/Thunder_Night.png";>;
// Snow
Texture2D TexWLUT_Snow_Day         <string ResourceName="Textures/LUTs/Weather/Snow_Day.png";>;
Texture2D TexWLUT_Snow_DawnDusk    <string ResourceName="Textures/LUTs/Weather/Snow_DawnDusk.png";>;
Texture2D TexWLUT_Snow_Night       <string ResourceName="Textures/LUTs/Weather/Snow_Night.png";>;
// Ash
Texture2D TexWLUT_Ash_Day          <string ResourceName="Textures/LUTs/Weather/Ash_Day.png";>;
Texture2D TexWLUT_Ash_DawnDusk     <string ResourceName="Textures/LUTs/Weather/Ash_DawnDusk.png";>;
Texture2D TexWLUT_Ash_Night        <string ResourceName="Textures/LUTs/Weather/Ash_Night.png";>;
...
// =================== CAS Sharpening (AMD FidelityFX CAS) ===================
int _spc_CAS0 < string UIName = ""; string UIWidget = "spacer"; int UIMin = 0; int UIMax = 0; > = 0;
int _hdr_CAS  < string UIName = "=== CAS SHARPENING ==="; string UIWidget = "spacer"; int UIMin = 0; int UIMax = 0; > = 0;

bool  UI_CAS_Enable    < string UIName = "CAS | Enable"; > = {false};
float UI_CAS_Sharpness < string UIName = "CAS | Sharpness"; string UIWidget = "Spinner"; float UIMin = 0.0; float UIMax = 1.0; float UIStep = 0.05; > = {0.5};

// =================== Weather LUT System ===================
int _spc_WLUT0 < string UIName = ""; string UIWidget = "spacer"; int UIMin = 0; int UIMax = 0; > = 0;
int _hdr_WLUT  < string UIName = "=== WEATHER LUT ==="; string UIWidget = "spacer"; int UIMin = 0; int UIMax = 0; > = 0;

bool  UI_WLUT_Enable    < string UIName = "WLUT | Enable"; > = {false};
float UI_WLUT_Intensity < string UIName = "WLUT | Intensity"; string UIWidget = "Spinner"; float UIMin = 0.0; float UIMax = 1.0; float UIStep = 0.05; > = {0.8};
...
bool  LensDist_Enable() { return false; }
...
//----------------------------------------------------------------------------------------------//
//  Weather LUT Selection: maps weather category + time slot to a LUT texture
//----------------------------------------------------------------------------------------------//

float3 SampleWeatherLUT(float3 color, int weatherCat, int timeSlot, SamplerState smp)
{
    // 7 weather x 3 time = 21 cases, resolved via nested switch
    // Returns sampled LUT color; identity LUT returns input unchanged
    if (weatherCat == 0) // Clear
    {
        if (timeSlot == 0) return SB_SampleLUT(TexWLUT_Clear_Day, smp, color);
        if (timeSlot == 1) return SB_SampleLUT(TexWLUT_Clear_DawnDusk, smp, color);
        return SB_SampleLUT(TexWLUT_Clear_Night, smp, color);
    }
    if (weatherCat == 1) // Cloudy
    {
        if (timeSlot == 0) return SB_SampleLUT(TexWLUT_Cloudy_Day, smp, color);
        if (timeSlot == 1) return SB_SampleLUT(TexWLUT_Cloudy_DawnDusk, smp, color);
        return SB_SampleLUT(TexWLUT_Cloudy_Night, smp, color);
    }
    if (weatherCat == 2) // Foggy
    {
        if (timeSlot == 0) return SB_SampleLUT(TexWLUT_Foggy_Day, smp, color);
        if (timeSlot == 1) return SB_SampleLUT(TexWLUT_Foggy_DawnDusk, smp, color);
        return SB_SampleLUT(TexWLUT_Foggy_Night, smp, color);
    }
    if (weatherCat == 3) // Rain
    {
        if (timeSlot == 0) return SB_SampleLUT(TexWLUT_Rain_Day, smp, color);
        if (timeSlot == 1) return SB_SampleLUT(TexWLUT_Rain_DawnDusk, smp, color);
        return SB_SampleLUT(TexWLUT_Rain_Night, smp, color);
    }
    if (weatherCat == 4) // Thunder
    {
        if (timeSlot == 0) return SB_SampleLUT(TexWLUT_Thunder_Day, smp, color);
        if (timeSlot == 1) return SB_SampleLUT(TexWLUT_Thunder_DawnDusk, smp, color);
        return SB_SampleLUT(TexWLUT_Thunder_Night, smp, color);
    }
    if (weatherCat == 5) // Snow
    {
        if (timeSlot == 0) return SB_SampleLUT(TexWLUT_Snow_Day, smp, color);
        if (timeSlot == 1) return SB_SampleLUT(TexWLUT_Snow_DawnDusk, smp, color);
        return SB_SampleLUT(TexWLUT_Snow_Night, smp, color);
    }
    // weatherCat == 6: Ash
    if (timeSlot == 0) return SB_SampleLUT(TexWLUT_Ash_Day, smp, color);
    if (timeSlot == 1) return SB_SampleLUT(TexWLUT_Ash_DawnDusk, smp, color);
    return SB_SampleLUT(TexWLUT_Ash_Night, smp, color);
}

float3 ApplyWeatherLUTSystem(float3 color, SamplerState smp, float intensity)
{
    int weatherCat = SB_GetWeatherCategory();
    int timeSlot   = SB_GetTimeSlot();

    float3 currentGraded = SampleWeatherLUT(color, weatherCat, timeSlot, smp);

    // Blend with previous weather during transitions
    float transition = SB_Weather_Transition.x;
    if (transition < 0.99)
    {
        // Use Clear as fallback for previous weather (outgoing weather category
        // isn't directly exposed — transition.x blends toward current)
        float3 prevGraded = SampleWeatherLUT(color, 0, timeSlot, smp);
        currentGraded = lerp(prevGraded, currentGraded, transition);
    }

    return lerp(color, currentGraded, intensity);
}
...
float4 PS_ColorGrade(CGVSOutput IN) : SV_Target
{
    ...
    // Weather LUT: auto-select and blend based on SkyrimBridge weather + time data
    [branch] if (UI_WLUT_Enable && SB_IsActive())
        result = ApplyWeatherLUTSystem(result, Linear_Sampler, UI_WLUT_Intensity);
...
// Dither UI params (UID_Intensity, UID_Motion, UID_Visualize) provided by enbUI_PostPass.fxh
...
float4 PS_Final(FinalVSOutput IN) : SV_Target
{
    ...
    // === 0. CAS Sharpening (first: reads neighbors from TextureColor before any per-pixel effects) ===
    [branch] if (UI_CAS_Enable)
        color = SB_ApplyCAS(TextureColor, Point_Sampler, UV, PixelSize.xy, UI_CAS_Sharpness);
