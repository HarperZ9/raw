...
//----------------------ENB PRESET---------------------//
//         MM"""""""`YM MM"""""""`MM MM'""""'YMM       //
//         MM  mmmmm  M MM  mmmm,  M M' .mmm. `M       // 
//         M'        .M M'        .M M  MMMMMooM       // 
//         MM  MMMMMMMM MM  MMMb. "M M  MMMMMMMM       // 
//         MM  MMMMMMMM MM  MMMMM  M M. `MMM' .M       // 
//         MM  MMMMMMMM MM  MMMMM  M MM.     .dM       // 
...
/* float fUnblur 
< 
	string UIName="Unblur distance"; 
	string UIWidget="spinner"; 
	float UIMin=0.0; 
	float UIMax=10.0; 
> = {1.5}; */

bool	bAutoFocus				// Fixed focus point or Manually placed focus point
<
	string UIName="Dof:: Auto Focus";
> = {true};	// true
...
//------ New paramaters for finer control of the focus point



/* float	fFocusX				// Where on horizontal screen is the fixed focus point
<
	string UIName="Dof:: Focus Point X";
	string UIWidget="spinner";
	float UIMin=0.0;
	float UIMax=1.0;
> = {0.5};	// 0.5

float	fFocusY				// Where on vertical screen is the foxed focus point
<
	string UIName="Dof:: Focus Point Y";
	string UIWidget="spinner";
	float UIMin=0.0;
	float UIMax=1.0;
> = {0.5};	// 0.5 */
...
float4	PS_ReadFocus(VS_OUTPUT_POST IN, float4 v0 : SV_Position0) : SV_Target
{
	...
	for (int x=0; x<16; x++)
	{
		...
		for (int y=0; y<16; y++)
		{
			...
			{
			coord=float2(0.5, 0.5);	// If autofocus, coord refers to X and Y
			...
			float	tempcurr=TextureDepth.SampleLevel(Sampler0, coord, 0.5).x;
			*/
...
if (bUnblurPlayer == true)
{
	const float	fpdistance=1.0/fUnblur;
...
technique11 Dof <string UIName="Boris DOF-PRC LITE edited"; string RenderTarget="RenderTargetR16F";>
{
	pass p0
	{
		SetVertexShader(CompileShader(vs_5_0, VS_Quad()));
