...
namespace D3D11Hook
{
    ...
    static void InstallCursorHook()
    {
        ...
        if (patchCount > 0) {
            SKSE::log::info("Playground: SetCursorPos IAT hook installed ({} module(s) patched)", patchCount);
        } else {
            SKSE::log::warn("Playground: SetCursorPos not found in any module IAT — cursor may snap during GUI");
        }
    }

    static void UnclipCursor()
    {
        // Save the current clip rect so we can restore it
        if (!s_hasClipRect) {
            GetClipCursor(&s_savedClipRect);
            s_hasClipRect = true;
        }
        ClipCursor(nullptr);  // remove clip — free mouse movement
    }
    ...
    static LRESULT CALLBACK HookedWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
    {
        ...
        if (uMsg == WM_KEYDOWN && wParam == VK_F9) {
            ...
            SKSE::log::info("Playground: Render passes {} (F9)",
        ...
        if (uMsg == WM_KEYDOWN && wParam == VK_F8) {
            ...
            SKSE::log::info("Playground: Compute passes {} (F8)",
        ...
        if (uMsg == WM_KEYDOWN && wParam == VK_F7) {
            ...
            SKSE::log::info("Playground: Mid-frame dispatch {} (F7)",
        ...
        if (uMsg == WM_KEYDOWN && wParam == VK_F10) {
            ...
            if (cap.IsCapturing()) {
                cap.StopCapture();
                SKSE::log::info("Playground: Frame capture STOPPED manually (F10)");
            } else {
                cap.StartCapture(600);
                SKSE::log::info("Playground: Frame capture STARTED — 600 frames (F10)");
            }
        }
        ...
        if (uMsg == WM_KEYDOWN && wParam == VK_F11) {
            ...
            if (prof.IsInitialized()) {
                ...
                SKSE::log::info("Playground: GPU Profiler {} (F11)",
        ...
        if (s_guiVisible && s_imguiInitialized) {
            // Keep cursor unclipped every frame — Skyrim re-clips it constantly
            ClipCursor(nullptr);
    ...
    static bool InitImGui()
    {
        ...
        if (!ImGui_ImplWin32_Init(s_gameWindow)) {
            SKSE::log::error("Playground: ImGui_ImplWin32_Init failed");
            return false;
        }

        if (!ImGui_ImplDX11_Init(s_device, s_context)) {
            SKSE::log::error("Playground: ImGui_ImplDX11_Init failed");
            ImGui_ImplWin32_Shutdown();
            return false;
        }

        s_imguiInitialized = true;
        SKSE::log::info("Playground: ImGui initialized successfully");
        return true;
    }
    ...
    static bool CreateRenderTarget()
    {
        ...
        if (FAILED(hr)) {
            SKSE::log::error("Playground: Failed to get back buffer");
            return false;
        }
        ...
        if (FAILED(hr)) {
            SKSE::log::error("Playground: Failed to create render target view");
            return false;
        }
    ...
    static void DoOverlayWork(IDXGISwapChain* swapChain)
    {
        ...
        // Frame Capture: end previous frame, start new
        {
            ...
            if (capture.IsCapturing()) {
                ...
                // Feed proxy stats
                auto& proxy = SB::ProxyDiagnostics::Get();
                if (proxy.IsConnected()) {
                    auto& snap = proxy.GetSnapshot();
                    capture.SetProxyStats(snap.drawCalls, snap.rtSwitches, snap.phaseId);
                }
            }
        }
        ...
        // Execute PrePresent pipeline passes (each pass has its own enabled flag)
        // These now operate on the pre-UI scene (if available), not the UI-contaminated buffer
        {
        ...
        // Read feedback (read-only — samples backbuffer pixels)
        SB::FeedbackProcessor::Get().CollectFeedback(s_context, swapChain);
        ...
        if (!s_imguiInitialized && s_device && s_context && s_gameWindow) {
            if (CreateRenderTarget() && InitImGui()) {
                SKSE::log::info("Playground: ImGui initialized in Present hook");
            }
        }
        ...
        if (s_guiVisible && s_imguiInitialized && s_renderTargetView) {
            ...
            // may never reach us if ENB or another hook consumes them first.
        ...
        if (!s_overlayDisabled) {
            ...
            __except (EXCEPTION_EXECUTE_HANDLER) {
                ...
                if (s_overlayErrorCount >= 3) {
                    ...
                    SKSE::log::error("Playground: overlay crashed {} times — auto-disabled to protect game stability",
                        s_overlayErrorCount);
                }
            }
        }
        ...
        if (!s_overlayDisabled) {
            ...
            __except (EXCEPTION_EXECUTE_HANDLER) {
                ...
                if (s_overlayErrorCount >= 3) {
                    ...
                    SB::BootDiag::DumpReport();
                    SKSE::log::error("Playground: overlay crashed {} times — auto-disabled",
                        s_overlayErrorCount);
                }
            }
        }
    ...
    static bool TryProxyInit()
    {
        ...
        if (!getProxy) {
            SKSE::log::info("Playground: d3d11.dll has no SB_GetProxyInterface — not our proxy");
            return false;
        }

        s_proxy = getProxy();
        if (!s_proxy || s_proxy->version < 1) {
            SKSE::log::error("Playground: Proxy interface invalid (version={})",
        ...
        if (!s_device || !s_context || !s_swapChain) {
            SKSE::log::error("Playground: Proxy has null D3D11 objects — device={}, ctx={}, sc={}",
        ...
        s_proxyMode = true;
        SKSE::log::info("Playground: Connected to d3d11 proxy v{}", s_proxy->version);
    ...
    static bool HookGameSwapChain()
    {
        ...
        if (!renderer) {
            SKSE::log::error("Playground: BSGraphics::Renderer not available");
            return false;
        }
        ...
        if (!s_swapChain || !s_device || !s_context || !s_gameWindow) {
            SKSE::log::error("Playground: Null D3D11 objects from renderer");
            return false;
        }

        SKSE::log::info("Playground: Got game swap chain, device, and window (legacy mode)");
        ...
        if (!s_originalWndProc && GetLastError() != 0) {
            SKSE::log::error("Playground: Failed to hook window procedure (error {})", GetLastError());
            return false;
        }
        if (!s_originalWndProc) {
            s_originalWndProc = DefWindowProcA;
            SKSE::log::warn("Playground: Previous WndProc was null — using DefWindowProc fallback");
        }
        ...
        if (VirtualProtect(&s_swapChainVTable[8], sizeof(void*), PAGE_EXECUTE_READWRITE, &oldProtect)) {
            ...
            SKSE::log::info("Playground: Present hook installed via vtable (legacy)");
        } else {
            SKSE::log::error("Playground: VirtualProtect failed");
            return false;
        }
    ...
    static bool InitWindowHook()
    {
        ...
        if (!s_gameWindow) {
            SKSE::log::error("Playground: Game window is null");
            return false;
        }
    ...
    bool Init()
    {
        ...
        if (TryProxyInit()) {
            SKSE::log::info("Playground: Using PROXY mode (d3d11.dll proxy active)");
            InitWindowHook();
            s_initialized = true;
            SKSE::log::info("Playground: D3D11 hook initialized (proxy) — press INSERT to toggle GUI");
            return true;
        }

        // Fall back to legacy vtable hook (vanilla d3d11)
        SKSE::log::info("Playground: No proxy detected — using LEGACY mode (vtable hook)");
        if (!HookGameSwapChain()) {
            SKSE::log::error("Playground: Failed to hook game swap chain");
            return false;
        }

        s_initialized = true;
        SKSE::log::info("Playground: D3D11 hook initialized (legacy) — press INSERT to toggle GUI");
        return true;
    }
    ...
    void Shutdown()
    {
        ...
        SB::TemporalSuperRes::Get().Shutdown();
        SB::FrameGenerator::Get().Shutdown();
        ...
        SB::DebugRenderer::Get().Shutdown();
        SB::PipelineTest::Shutdown();
    ...
    void ToggleGUI()
    {
        ...
        SKSE::log::info("Playground: GUI toggled {}", s_guiVisible ? "ON" : "OFF");
    }
