...
void CSCopy(uint3 DTid : SV_DispatchThreadID)
{
    if (DTid.x >= DstDimensions.x || DTid.y >= DstDimensions.y)
        return;
    // Convert standard depth to reversed-Z for precision.
    // We own the depth buffer — no undrawn pixels to worry about.
...
namespace SB
{
    ...
    bool HiZPyramid::CompileComputeShaders(ID3D11Device* a_device)
    {
        ...
        // Compile copy CS
        blob = ShaderLoader::Compile("HiZ_Copy", kCopyCS, "CSCopy", "cs_5_0", 0);
        if (!blob) {
        ...
        blob = ShaderLoader::Compile("HiZ_Downsample", kDownsampleCS, "CSDownsample", "cs_5_0", 0);
        if (!blob) {
    ...
    void HiZPyramid::BuildPyramid(ID3D11DeviceContext* a_ctx)
    {
        ...
        // Unbind ALL OM targets + PS SRVs that might reference the depth texture.
        // D3D11 silently zeros SRV reads if the same resource is bound as DSV
        // anywhere in the pipeline — even if we think we unbound it.
        {
            ID3D11RenderTargetView* nullRTVs[8] = {};
            a_ctx->OMSetRenderTargets(8, nullRTVs, nullptr);
            // Clear PS SRV slots 0-19 to break any depth texture binding
            ID3D11ShaderResourceView* nullSRVs[20] = {};
            a_ctx->PSSetShaderResources(0, 20, nullSRVs);
        }

        // Acquire depth buffer SRV.
        // PREFER proxy depth (DepthIntercept) — it has the full scene.
        // AcquireDepthSRV reads the OM-bound DSV which at Present time
        // may only contain first-person data (game clears between passes).
        {
            auto* proxyDepth = D3D11Hook::GetGameDepthSRV();
            if (proxyDepth) {
                if (m_depthSRV && m_ownDepthSRV) { m_depthSRV->Release(); }
                m_depthSRV = proxyDepth;
                m_ownDepthSRV = false;
            } else if (!AcquireDepthSRV(a_ctx)) {
                static uint32_t s_depthFail = 0;
                if (s_depthFail++ % 300 == 0)
                    SKSE::log::warn("HiZPyramid: no depth SRV — proxy null and AcquireDepthSRV failed (frame {})", s_depthFail);
                return;
            }
        }
