...
private:
    ...
    // DSV-unbind geometry-end detection
    // Tracks consecutive OMSetRenderTargets calls where the main DSV is NOT bound.
    // Mid-geometry fullscreen passes unbind the DSV for 1-2 calls then rebind.
    // Real PostProcess never rebinds. Threshold of 3 distinguishes the two.
    bool     m_mainDSVCurrentlyBound = false;
    uint32_t m_rtSwitchesSinceMainDSV = 0;
    static constexpr uint32_t kGeometryEndRTThreshold = 3;
